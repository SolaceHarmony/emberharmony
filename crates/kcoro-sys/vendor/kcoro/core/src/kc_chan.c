...
static void kc_chan_schedule_wake(struct kc_wake wake)
{
    ...
    /* Patch 0001: always route through kcoro_unpark — if the target is parked this
     * readies it; if it is still inside the park window (state RUNNING) it stores the
     * wake as a park_notify permit instead of dropping it. The enqueue below remains
     * for suspended/yielded waiters and is deduped by ready_enqueued. */
    kcoro_unpark(co);
    if (!current) {
