...
// y[i] = a[i] * b[i]    — elementwise (upsample channel-vector product)
static inline void vmul(float *y, const float *a, const float *b, int n) {
#if MIMI_NEON
    int i = 0;
    for (; i + 4 <= n; i += 4)
...
static void weight_norm_fold(const float *v, const float *g, float *out,
    ...
    for (int c = 0; c < n0; ++c) {
        ...
        const float scale = g[c] / sqrtf(ss);
...
int mimi_conv_step(MimiConvState *s, const float *xs, int n_in, float *y) {
    ...
    if (num_frames > 0) {
        ...
        for (int oc = 0; oc < s->out_c; ++oc) {
            ...
            if (s->bias) vadd_scalar(yrow, s->bias[oc], nf);  // candle: +bias last
        }
...
int mimi_convtr_step(MimiConvTrState *s, const float *xs, int n_in, float *y) {
    ...
    for (int oc = 0; oc < oc_n; ++oc) {
        ...
        if (s->bias) {
            vadd_scalar(yrow, s->bias[oc], emit_len);
            if (invalid > 0) vadd_scalar(crow, s->bias[oc], invalid);
        }
...
// vector prod[c] = X[c] * wrep[kk][c] with a NEON elementwise multiply (wrep is
// the weight repacked once at init to [k, dim], channel-contiguous per tap),
// then scatter prod to output time out_t=kk. n_in>1 (not on the decode path)
// falls back to a per-channel NEON scale over the time axis.
...
struct MimiUpsampleState {
    ...
    float *wrep;         // [ksize, dim] repacked (channel-contiguous per tap)
    float *prev;         // [dim, invalid] overlap carry (no bias)
    float *carry_scratch;// [dim, invalid]
    float *prod;         // [dim] channel-vector product scratch
...
int mimi_upsample_init(MimiUpsampleState **st, const MimiWeightTable *w,
    ...
    for (int c = 0; c < dim; ++c)
        for (int kk = 0; kk < ksize; ++kk)
            s->wrep[(size_t)kk * dim + c] = s->w[(size_t)c * ksize + kk];
    ...
    s->prod = (float *)mimi_arena_alloc(a, (size_t)dim * sizeof(float));
    s->g = (float *)mimi_arena_alloc(a, (size_t)MIMI_CONV_MAX_NIN * sizeof(float));
    s->prev_valid = 0;
    *st = s;
    return 0;
...
int mimi_upsample_step(MimiUpsampleState *s, const float *xs, int n_in, float *y) {
    ...
    if (n_in == 1) {
        ...
        // prod = X (*) wrep[kk] is one NEON multiply over the channel axis. With
        ...
        for (int kk = 0; kk < k; ++kk) {
            vmul(s->prod, xs, s->wrep + (size_t)kk * dim, dim);
            const int out_t = kk;                   // l==0
            if (out_t < emit_len) {
                for (int c = 0; c < dim; ++c) y[(size_t)c * emit_len + out_t] = s->prod[c];
            } else {
                const int ci = out_t - emit_len;
                for (int c = 0; c < dim; ++c) s->carry_scratch[(size_t)c * invalid + ci] = s->prod[c];
            }
        }
    } else {
        ...
        for (int c = 0; c < dim; ++c) {
            for (int kk = 0; kk < k; ++kk) {
                vscale(g, xs + (size_t)c * n_in, s->w[(size_t)c * k + kk], n_in);
 ...
 *               prod = X (*) wrep[kk] (NEON vmul), scattered to out_t=kk. Depthwise
 ...
 *                disjoint writes — shareable, or slice per band). SHARED: wrep,
