...
mod mel {
    ...
    //!
    //! This rim owns the opaque native plan/workspace lifetimes and the final
    //! transport into a candle tensor for the not-yet-native Conformer. Realtime
    //! borrows retained PCM directly and requests valid-only output; the one
    //! remaining host-to-model-device mel handoff dies with the Conformer
    //! cutover (spec 11 doc 06). It is transport, not math.
