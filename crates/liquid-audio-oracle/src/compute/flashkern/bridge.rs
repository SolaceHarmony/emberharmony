...
extern "C" {
    ...
    fn lfm_kernel_bridge_submit_borrowed(bridge: *mut c_void, submission: *const Submission)
        -> i32;
    fn lfm_kernel_bridge_try_submission(bridge: *mut c_void, out: *mut Submission) -> i32;
    ...
    fn lfm_kernel_bridge_try_completion(bridge: *mut c_void, out: *mut Completion) -> i32;
...
impl Bridge {
    ...
    fn try_submission(&self) -> Result<Submission, i32> {
        let mut out = MaybeUninit::<Submission>::uninit();
        let rc = unsafe { lfm_kernel_bridge_try_submission(self.0, out.as_mut_ptr()) };
        if rc != 0 {
            return Err(rc);
        }
    ...
    fn try_completion(&self) -> Result<Completion, i32> {
        let mut out = MaybeUninit::<Completion>::uninit();
        let rc = unsafe { lfm_kernel_bridge_try_completion(self.0, out.as_mut_ptr()) };
        if rc != 0 {
            return Err(rc);
        }
        Ok(unsafe { out.assume_init() })
...
#[test]
fn nonblocking_endpoints_distinguish_empty_from_settled_stop() {
    let bridge = Bridge::new(1);
    assert_eq!(bridge.try_submission().unwrap_err(), -libc::EAGAIN);
    assert_eq!(bridge.try_completion().unwrap_err(), -libc::EAGAIN);

    let lease = bridge.descriptor(17);
    let input = submission(1, lease.id);
    assert_eq!(bridge.submit(&input), 0);
    let dispatched = bridge.try_submission().unwrap();
    assert_eq!(dispatched.ticket.sequence, input.ticket.sequence);
    assert_eq!(bridge.try_submission().unwrap_err(), -libc::EAGAIN);
    assert_eq!(bridge.try_completion().unwrap_err(), -libc::EAGAIN);

    let output = completion(&dispatched, 1);
    assert_eq!(bridge.publish(&output), 0);
    let settled = bridge.try_completion().unwrap();
    assert_eq!(settled.ticket.sequence, input.ticket.sequence);
    bridge.request_stop();
    assert_eq!(bridge.try_submission().unwrap_err(), -libc::ECANCELED);
    assert_eq!(bridge.try_completion().unwrap_err(), -libc::ECANCELED);
}
