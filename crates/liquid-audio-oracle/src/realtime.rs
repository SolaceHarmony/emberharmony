...
mod tests {
    ...
    fn frame_pipeline_interrupt_drops_stale_frames_without_reset() {
        ...
        while pipe.submit_frame(vec![0.1, 0.1]) {
            queued += 1;
            assert!(queued < 64, "frame queue must remain bounded");
        }
        pipe.interrupt();
        release_tx.send(()).unwrap();
    ...
    fn shutdown_skips_queued_prepare() {
        ...
        let (entered_tx, entered_rx) = mpsc::channel();
        let (release_tx, release_rx) = mpsc::channel();
        let mut pipe = RealtimePipeline::spawn(RecordingEngine::blocking(
            log.clone(),
            entered_tx,
            release_rx,
        ))
