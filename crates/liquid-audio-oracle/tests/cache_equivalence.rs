...
fn collect_tokens<'a>(
    ...
    mods: &'a mut Vec<i64>,
) -> impl FnMut(liquid_audio_oracle::GenToken) + 'a {
    move |tok| match tok {
        liquid_audio_oracle::GenToken::Text(id) => {
            toks.push((id as i64, Vec::new()));
            mods.push(liquid_audio_oracle::LFMModality::Text as i64);
        }
        liquid_audio_oracle::GenToken::Audio(frame) => {
            toks.push((-1, frame));
            mods.push(liquid_audio_oracle::LFMModality::AudioOut as i64);
        }
    }
}
...
fn suffix_cache_matches_full_prefill() -> anyhow::Result<()> {
    use liquid_audio_oracle::{ChatState, GenParams, LFMModality, PrefillCursor};
    ...
    let (model, proc) = liquid_audio_oracle::from_pretrained(Path::new(&dir), &device)?;
