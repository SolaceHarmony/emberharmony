...
use liquid_audio_oracle::model::lfm2_hf::Cache as LfmCache;
use liquid_audio_oracle::moshi::demo::chat::decode_audio_reply;
use liquid_audio_oracle::moshi::models::MimiModel;
use liquid_audio_oracle::{ChatState, GenParams, GenToken, LFM2AudioModel, LFMModality, PrefillCursor};
...
fn run_conversation(
    ...
    let (model, proc) = liquid_audio_oracle::from_pretrained(dir, device).expect("load model");
