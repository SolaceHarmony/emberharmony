...
use liquid_audio_oracle::processor::{FilterbankFeatures, MelConfig};
...
unsafe extern "C" {
    ...
    fn lfm_resampler_stream_create(
        ...
        new_freq: u32,
        max_sample_count: u64,
        out: *mut *mut NativeResamplerStream,
    ) -> i32;
    fn lfm_resampler_stream_destroy(stream: *mut NativeResamplerStream) -> i32;
    fn lfm_resampler_stream_reset(stream: *mut NativeResamplerStream);
    fn lfm_resampler_stream_out_length(
        stream: *mut NativeResamplerStream,
        sample_count: u64,
        out_length: *mut u64,
    ) -> i32;
...
fn resampler_matches_deleted_rust() {
    ...
    for (name, orig, new) in [
        ...
        let got = liquid_audio_oracle::resample::resample_slice(&input, orig, new);
...
fn prepared_resampler_borrows_equal_rate_and_writes_different_rate_directly() {
    ...
    unsafe {
        ...
        // A direct view cannot preserve unread source values when its output
        // aliases that source. Reject the overlap instead of silently staging.
        let mut overlap = input.clone();
        let overlap_input = overlap.as_ptr();
        let overlap_output = overlap.as_mut_ptr();
        assert_ne!(
            lfm_resampler_process(
                down,
                down_ws,
            ...
            0,
            "different-rate resample must reject overlapping spans"
        );
...
#[test]
fn direct_resampler_leaf_partitions_arbitrary_output_ranges_bitwise() {
    let input = [-0.75f32, 0.25, 1.0, -0.125, 0.5, -1.0, 0.375, 0.625, -0.25];
    const PHASES: usize = 2;
    const STRIDE: usize = 3;
    const WIDTH: usize = 2;
    const KLEN: usize = 2 * WIDTH + STRIDE;
    let kernels = [
        0.125f64, -0.25, 0.5, 0.75, -0.375, 0.0625, 0.25, -0.1875, 0.375, -0.625, 0.5, 0.25,
        -0.125, 0.03125,
    ];
    assert_eq!(kernels.len(), PHASES * KLEN);

    let mut whole = [f32::NAN; 6];
    let mut split = [f32::NAN; 7];
    split[6] = f32::from_bits(0x7f12_3456);
    unsafe {
        lfm_resample_conv_f32(
            input.as_ptr(),
            input.len() as u64,
            kernels.as_ptr(),
            PHASES as u64,
    ...
    let split_bits: [u32; 6] = std::array::from_fn(|index| split[index].to_bits());
    assert_eq!(whole.map(f32::to_bits), split_bits);
    assert_eq!(split[6].to_bits(), 0x7f12_3456);
}

#[test]
fn streaming_playback_resampler_retains_boundary_history_and_full_geometry() {
    unsafe {
        let mut whole = std::ptr::null_mut();
        let mut chunked = std::ptr::null_mut();
        assert_eq!(
            lfm_resampler_stream_create(24_000, 48_000, 3_840, &mut whole),
            0
        );
        assert_eq!(
            lfm_resampler_stream_create(24_000, 48_000, 3_840, &mut chunked),
            0
        );

        let input = [0.0f32, 1.0, 0.0, -1.0, 0.25];
        let expected = [0.0f32, 0.0, 0.5, 1.0, 0.5, 0.0, -0.5, -1.0, -0.375, 0.25];
        let mut contiguous = [f32::NAN; 10];
        let mut written = NativeF32Span {
            data: std::ptr::null(),
            length: 0,
        };
        ...
        assert_eq!(written.data, contiguous.as_ptr());
        assert_eq!(written.length, contiguous.len() as u64);
        assert_eq!(contiguous.map(f32::to_bits), expected.map(f32::to_bits));

        let mut first = [f32::NAN; 4];
        assert_eq!(
            lfm_resampler_stream_process(
                chunked,
                input.as_ptr(),
        ...
        let split = [
            first[0], first[1], first[2], first[3], second[0], second[1], third[0], third[1],
            third[2], third[3],
        ];
        assert_eq!(split.map(f32::to_bits), contiguous.map(f32::to_bits));

        lfm_resampler_stream_reset(chunked);
        let mut replay = [f32::NAN; 10];
        assert_eq!(
            lfm_resampler_stream_process(
                chunked,
                input.as_ptr(),
        ...
        assert_eq!(replay.map(f32::to_bits), contiguous.map(f32::to_bits));

        lfm_resampler_stream_reset(chunked);
        let codec = vec![0.0f32; 3_840];
        let mut frames = 0;
        assert_eq!(
            lfm_resampler_stream_out_length(chunked, codec.len() as u64, &mut frames),
            0
        );
        assert_eq!(frames, 7_680);
        let mut playback = vec![f32::NAN; frames as usize];
        assert_eq!(
            lfm_resampler_stream_process(
                chunked,
                codec.as_ptr(),
        ...
        assert_eq!(written.data, playback.as_ptr());
        assert_eq!(written.length, 7_680);

        assert_eq!(lfm_resampler_stream_destroy(chunked), 0);
        assert_eq!(lfm_resampler_stream_destroy(whole), 0);
    }
}

#[test]
fn streaming_playback_resampler_preserves_fractional_phase_across_blocks() {
    unsafe {
        let mut whole = std::ptr::null_mut();
        let mut chunked = std::ptr::null_mut();
        assert_eq!(
            lfm_resampler_stream_create(24_000, 44_100, 3_840, &mut whole),
            0
        );
        assert_eq!(
            lfm_resampler_stream_create(24_000, 44_100, 3_840, &mut chunked),
            0
        );

        let input = [
            0.0f32, 0.25, -0.5, 1.0, -1.0, 0.75, -0.25, 0.125, -0.875, 0.5, 0.375, -0.625, 0.0,
        ];
        let mut frames = 0;
        assert_eq!(
            lfm_resampler_stream_out_length(whole, input.len() as u64, &mut frames),
            0
        );
        let mut contiguous = vec![f32::NAN; frames as usize];
        let mut written = NativeF32Span {
            data: std::ptr::null(),
            length: 0,
        };
        ...
        assert_eq!(written.length, frames);

        let mut split = Vec::new();
        for part in [&input[..3], &input[3..4], &input[4..9], &input[9..]] {
            let mut count = 0;
            assert_eq!(
                lfm_resampler_stream_out_length(chunked, part.len() as u64, &mut count),
                0
            );
            let mut output = vec![f32::NAN; count as usize];
            assert_eq!(
                lfm_resampler_stream_process(
                    chunked,
                    part.as_ptr(),
            ...
            assert_eq!(written.data, output.as_ptr());
            assert_eq!(written.length, count);
            split.extend_from_slice(&output);
        }
        assert_eq!(split.len(), contiguous.len());
        assert_eq!(
            split
                .iter()
