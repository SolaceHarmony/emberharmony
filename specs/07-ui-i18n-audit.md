...
## Decision: How We Should Add i18n To `@thesolaceproject/emberharmony-ui`
...
- Export them from `@thesolaceproject/emberharmony-ui` via `packages/ui/package.json` exports (e.g. `"./i18n/*": "./src/i18n/*.ts"`).
...
- `packages/app/`:
  ...
  - Add a tiny bridge provider in `packages/app/src/app.tsx` to feed `useLanguage()` into `@thesolaceproject/emberharmony-ui`'s `I18nProvider`.
...
- `packages/enterprise/`:
  ...
  - Merge `@thesolaceproject/emberharmony-ui` dictionaries and (optionally) enterprise-local dictionaries.
...
1. Completed (2026-01-20): Add `@thesolaceproject/emberharmony-ui` i18n context (`packages/ui/src/context/i18n.tsx`) + export it.
