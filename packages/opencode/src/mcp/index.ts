...
export namespace MCP {
  ...
  async function fetchPromptsForClient(clientName: string, client: Client) {
    const prompts = await client.listPrompts().catch((e) => {
      ...
      const unsupported = msg.includes("-32601") || msg.includes("Method not found")
      if (unsupported) {
        log.debug("mcp prompts not supported", { clientName })
        return undefined
      }
      log.error("failed to get prompts", { clientName, error: msg })
      return undefined
    })
  ...
  async function fetchResourcesForClient(clientName: string, client: Client) {
    const resources = await client.listResources().catch((e) => {
      ...
      const unsupported = msg.includes("-32601") || msg.includes("Method not found")
      if (unsupported) {
        log.debug("mcp resources not supported", { clientName })
        return undefined
      }
      log.error("failed to get resources", { clientName, error: msg })
      return undefined
    })
  ...
  async function create(key: string, mcp: Config.Mcp) {
        ...
        try {
          ...
          if (error instanceof UnauthorizedError) {
            ...
            if (lastError.message.includes("registration") || lastError.message.includes("client_id")) {
              ...
              Bus.publish(TuiEvent.ToastShow, {
                title: "MCP Authentication Required",
                message: `Server "${key}" requires authentication. Run: code-harmony mcp auth ${key}`,
                variant: "warning",
                duration: 8000,
              }).catch((e) => log.debug("failed to show toast", { error: e }))
            }
    ...
    if (mcp.type === "local") {
      ...
      const transport = new StdioClientTransport({
        ...
        env: {
          ...process.env,
          ...(cmd === "code-harmony" || cmd === "opencode" ? { BUN_BE_BUN: "1" } : {}),
          ...mcp.environment,
        },
      })
