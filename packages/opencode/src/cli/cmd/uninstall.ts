...
export const UninstallCommand = {
  command: "uninstall",
  describe: "uninstall code-harmony and remove all related files",
  builder: (yargs: Argv) =>
    yargs
      .option("keep-config", {
        alias: "c",
  ...
  handler: async (args: UninstallArgs) => {
    ...
    UI.println(UI.logo("  "))
    UI.empty()
    prompts.intro("Uninstall OpenCode Harmony")
...
async function showRemovalSummary(targets: RemovalTargets, method: Installation.Method) {
  ...
  if (method !== "curl" && method !== "unknown") {
    const cmds: Record<string, string> = {
      npm: "npm uninstall -g code-harmony",
      pnpm: "pnpm uninstall -g code-harmony",
      bun: "bun remove -g code-harmony",
      yarn: "yarn global remove code-harmony",
      choco: "choco uninstall code-harmony",
      scoop: "scoop uninstall code-harmony",
    }
...
async function executeUninstall(method: Installation.Method, targets: RemovalTargets) {
  ...
  if (method !== "curl" && method !== "unknown") {
    ...
    if (cmd) {
      ...
      const result =
        method === "choco"
          ? await $`echo Y | choco uninstall code-harmony -y -r`.quiet().nothrow()
  ...
  if (method === "curl" && targets.binary) {
    ...
    if (binDir.includes(".opencode") || binDir.includes(".code-harmony")) {
  ...
  UI.empty()
  prompts.log.success("Thank you for using OpenCode Harmony!")
}
...
async function cleanShellConfig(file: string) {
  ...
  let skip = false
  const markers = new Set(["# code-harmony", "# opencode"])
  const pathParts = [".code-harmony/bin", ".opencode/bin"]
  ...
  for (const line of lines) {
    ...
    if (skip) {
      skip = false
      if (pathParts.some((part) => trimmed.includes(part)) || trimmed.includes("fish_add_path")) {
        continue
      }
    }

    if (
      (trimmed.startsWith("export PATH=") && pathParts.some((part) => trimmed.includes(part))) ||
      (trimmed.startsWith("fish_add_path") &&
        (trimmed.includes(".code-harmony") || trimmed.includes(".opencode")))
    ) {
      continue
    }
