...
describe("desktop voice context boundary", () => {
  ...
  test("CoreAudio renders capture directly into a native generation-checked reservation", async () => {
    const api = await liquid("src/voice_api.rs")
    const runtime = await liquid("src/runtime/voice_runtime.rs")
    const header = await liquid("native/include/lfm_platform_audio.h")
    const native = await liquid("native/src/runtime/lfm_platform_audio.cpp")
    const input = between(native, "OSStatus input_callback(", "OSStatus output_callback(")

    expect(api).not.toContain("trait CaptureSink")
    expect(api).not.toContain("trait PlaybackSource")
    expect(header).toContain("publish directly into the session's retained PCM")
    expect(input).toContain("lfm_capture_producer_claim_chunk(")
    expect(input).toContain("lfm_capture_producer_resolve_chunk(")
    expect(input).toContain("render_input(")
    expect(input).toContain("spans[0].data")
    expect(input).toContain("lfm_capture_producer_commit_chunk(")
    expect(input).not.toContain("lfm_capture_producer_write_interleaved(")
    expect(native).not.toContain("AudioQueue")
    expect(native).not.toContain("std::thread")
    expect(native).not.toContain("pthread")
    expect(native).not.toContain("wait(")
    expect(native).not.toContain("sleep")
    expect(runtime).not.toContain("cpal")
  ...
  test("desktop LFM2 uses liquid-audio direct platform callbacks", async () => {
    ...
    expect(audio).toContain("default_platform_audio_config()")
    expect(audio).toContain("mount_platform_audio(device)")
    expect(audio).toContain("engine.start_platform_audio()")
    expect(audio).toContain("struct OwnerSession")
    expect(audio).not.toContain("thread_local!")
    expect(audio).not.toContain("DeviceStreams")
    expect(audio).not.toContain("cpal")
    expect(native).toContain("AudioUnitRender(audio->input")
    expect(native).toContain("kAudioUnitProperty_SetRenderCallback")
  ...
  test("desktop enables liquid-audio platform audio callbacks", async () => {
    ...
    expect(cargo.match(/liquid-audio = \{ path = "\.\.\/\.\.\/\.\.\/crates\/liquid-audio" \}/g)).toHaveLength(2)
    expect(cargo).not.toContain('features = ["metal"]')
    expect(cargo).not.toContain('features = ["audio-io"]')
    expect(cargo).not.toContain("candle-core")
    expect(lib).toContain("pub mod voice_runtime")
    expect(build).toContain('rustc-link-lib=framework=AudioUnit')
    expect(build).toContain('native/src/runtime/lfm_platform_audio.cpp')
    expect(voice).toContain("default_platform_audio_config()")
    expect(voice).not.toContain("cpal")
    expect(voice).not.toContain("audio-io")
  })
  ...
  test("desktop voice docs describe the implemented Tauri kernel command surface", async () => {
    ...
    expect(architecture).toContain("Native code owns platform callback endpoints")
