...
describe("desktop voice context boundary", () => {
  ...
  test("native audio callbacks use bounded rings instead of mutexed vec buffers", async () => {
    ...
    expect(text).toContain("wake: Arc<Doorbell>")
    expect(text).toContain("fn observe(&self) -> u32")
    expect(text).toContain("fn park(&self, expected: u32)")
    expect(text).toContain("self.wake.park(expected)")
    expect(text).toContain("self.write\n            .store(write.wrapping_add(count), Ordering::Release)")
  ...
  test("native mic loops wake from audio arrival instead of blind sleep polling", async () => {
    ...
    expect(vad).toContain("let expected = mic.observe()")
    expect(vad).toContain("mic.park(expected)")
    expect(vad).toContain("silent_frames >= silence_frames")
    expect(frame).toContain("let expected = mic.observe()")
    expect(frame).toContain("mic.park(expected)")
    expect(frame).toContain("let has_input = mic.len() > 0")
    expect(frame).toContain("if !has_input && mic.len() == 0")
    expect(input).toContain("mic.push_with")
    expect(vad).not.toContain("Duration::from_millis(40)")
    expect(frame).not.toContain("Duration::from_millis(10)")
    expect(frame).not.toContain("next_silence")
  ...
  test("native control commands wake sleeping mic loops immediately", async () => {
    ...
    expect(runtime).toContain("control: Arc<Doorbell>")
    expect(runtime).toContain("Arc::clone(&input.mic.wake)")
    expect(runtime).toContain("fn wake_control(&self)")
    expect(runtime).toContain("self.control.ring_all()")
    ...
    expect(input).toContain("pub fn push_mono_f32(&mut self")
    expect(input).not.toContain("self.mic.clear()")
  })
  ...
  test("desktop local LFM2 audio I/O is fed by WebRTC PlatformAudio", async () => {
    ...
    expect(audio).toContain("pub fn push_interleaved_i16(&mut self")
    expect(runtime).toContain("writer.push_interleaved_i16(frame.data.as_ref(), channels)")
    expect(runtime).not.toContain("audio_frame_to_mono_f32")
  ...
  test("native model progress stays native and sampler math is assembly-backed", async () => {
    const cargo = await liquid("Cargo.toml")
    const build = await liquid("build.rs")
    const kcoro = await root("crates/kcoro-sys/build.rs")
    const modules = await liquid("src/compute/flashkern/mod.rs")
    const native = await liquid("src/compute/flashkern/native_engine.rs")
    const linear = await liquid("src/model/linear.rs")
    const rustGemm = local("src/compute/bf16_gemm.rs")
    const engine = await liquid("native/src/engine/flashkern_engine.cpp")
    const bridge = await liquid("native/src/runtime/kernel_bridge.cpp")
    const protocol = await liquid("native/include/lfm_kernel_bridge.h")
    const neon = await liquid("native/kernels/aarch64/flashkern_neon.cpp")
    const x86 = await liquid("native/kernels/x86_64/flashkern_x86.cpp")
    const neonSampler = await liquid("native/kernels/aarch64/flashkern_sampler.S")
    const x86Sampler = await liquid("native/kernels/x86_64/flashkern_sampler.S")
    const ci = await root(".github/workflows/rust-voice.yml")
    const dev = cargo.indexOf("[dev-dependencies]")
    const rustKcoro = cargo.indexOf('kcoro = { path = "../kcoro" }')
    const symbols = [
      "lfm_sampler_argmax_f32",
      "lfm_sampler_argmax_bf16",
      "lfm_sampler_exp_sum_f32",
      "lfm_sampler_exp_sum_bf16",
      "lfm_sampler_prefix_pick",
    ]

    expect(cargo).toContain('kcoro-sys = { path = "../kcoro-sys" }')
    expect(dev).toBeGreaterThanOrEqual(0)
    expect(rustKcoro).toBeGreaterThan(dev)
    expect(build).toContain('.file("native/src/engine/flashkern_engine.cpp")')
    expect(build).toContain('.file("native/src/runtime/kernel_bridge.cpp")')
    expect(build).toContain('.file("native/kernels/aarch64/flashkern_sampler.S")')
    expect(build).toContain('.file("native/kernels/x86_64/flashkern_sampler.S")')
    ...
    expect(modules).not.toContain("mod coordinator")
    expect(native).toContain("kcoro_sys::link_anchor")
    expect(native).toContain("fn lfm_engine_new")
    expect(native).toContain("ptr: *mut c_void")
    expect(native).toContain("pass_lock: Mutex<()>")
    expect(native).toContain("pub fn process_engine()")
    expect(native).toContain("pub fn bf16_gemm_available()")
    expect(linear).toContain("bf16_gemm_f32(")
    expect(linear).not.toContain("CustomOp2")
    expect(await rustGemm.exists()).toBe(false)
    expect(native).not.toContain("Coordinator::new")
    expect(native).not.toContain("lfm_engine_set_submitter")
    expect(engine).not.toContain("e->submitter")
    expect(engine).toContain("lfm_kernel_bridge_submit")
    expect(engine).toContain("lfm_kernel_bridge_try_completion")
    expect(engine).toContain("lfm_kernel_bridge_try_submission")
    expect(engine).toContain("lfm_kernel_bridge_publish_completion")
    expect(engine).not.toContain("kc_ticket_")
    expect(engine).toContain("kc_runtime_create(&runtime_config")
    expect(engine).toContain("kc_service_create(e->control_runtime")
    expect(engine).toContain("kc_team_dispatch_notify")
    expect(engine).not.toContain("pthread_cond_")
    expect(bridge).toContain("kc_port_wait_u32")
    expect(bridge).toContain("kc_atomic_u32_fetch_add_release")
    expect(protocol).toContain("KcSubmissionV1")
    expect(protocol).toContain("KcCompletionV1")
    expect(protocol).toContain("KC_COORD_COMMAND_RUN_PASS")
    expect(engine).toContain("kc_port_wait_u32")
    expect(engine).toContain("kc_atomic_u32_fetch_add_release")
    expect(engine).not.toContain("pthread_create")
    expect(engine).not.toContain("pthread_join")
    ...
    expect(engine).toContain("std::vector<std::unique_ptr<BackbonePlan>> models")
    expect(engine).toContain("std::vector<std::unique_ptr<DepthPlan>> depth_plans")
    expect(engine).not.toContain("another model's table is live")
    expect(engine).toContain("REQ_DEPTHWISE_STREAM = 9")
    expect(engine).toContain("lfm_depthwise_stream_bf16_available()")
    expect(engine).toContain("run_depthwise_stream")
    expect(engine).toContain("REQ_FFT_CONV_DD = 11")
    expect(engine).toContain("fft_dd_lanes")
    expect(engine).toContain("for (size_t butterfly = lane;")
    expect(engine).toContain("ComplexDd *work = e->fft_work.data()")
    expect(engine).not.toContain("REQ_CALL")
    expect(engine).not.toContain("lfm_engine_call")
    expect(engine).not.toContain("lfm_lane_fence")
    expect(native).not.toContain("run_lanes")
    for (const symbol of symbols) {
      expect(neonSampler).toContain(`.globl LFM_SYM(${symbol})`)
      expect(x86Sampler).toContain(`.globl LFM_SYM(${symbol})`)
      expect(neon).not.toContain(symbol)
      expect(x86).not.toContain(symbol)
    }
    expect(neonSampler).toContain(".Lsampler_exp_f32")
    expect(x86Sampler).toContain(".Lsampler_exp_f32")
    expect(neonSampler).not.toContain("LFM_SYM(expf)")
    expect(x86Sampler).not.toContain("LFM_SYM(expf)")
    expect(ci).toContain("cargo test -p kcoro-sys --tests -- --nocapture")
  })

  test("documentation matches native recurrence and the assembly-only target", async () => {
    const native = await liquid("src/compute/flashkern/native_engine.rs")
    const weights = await liquid("src/compute/weights.rs")
    const engine = await liquid("native/src/engine/flashkern_engine.cpp")
    const loader = await liquid("native/src/io/safetensors.cpp")
    const map = await root("docs/native/README.md")
    const runbook = await root("docs/native/KCORO_ARENA_INTEGRATION.md")
    const design = await liquid("docs/ENGINE_DESIGN.md")
    const cpu = await liquid("docs/DECODE_ENGINE.md")
    const kernels = await liquid("docs/FLASHKERN.md")
    const deletion = await liquid("docs/RUST_DELETION_PLAN.md")
    const voice = await root("packages/desktop/src-tauri/src/voice/VOICE_ARCHITECTURE.md")
    const tickets = await root("specs/11-kcoro-native-migration/12-ticketed-orchestration-and-observability.md")
    const queue = engine.match(/constexpr size_t PASS_CAPACITY = (\d+);/)
    const descriptors = engine.match(/\.descriptor_capacity = (\d+),/)

    expect(queue?.[1]).toBe("2")
    expect(descriptors?.[1]).toBe("8")
    expect(engine).toContain("submission.command = KC_COORD_COMMAND_RUN_PASS")
    expect(engine).toContain("submission.service_class = KC_COORD_SERVICE_INTERACTIVE")
    expect(engine).toContain("submission.pass_budget = 1")
    expect(engine).toContain("lfm_kernel_bridge_submit")
    expect(engine).toContain("lfm_kernel_bridge_try_completion")
    expect(engine).toContain("kc_service_create(e->control_runtime")
    expect(engine).toContain("kc_team_dispatch_notify")
    ...
    expect(native).not.toContain("Coordinator::new")
    expect(native).not.toContain("lfm_engine_set_submitter")
    expect(engine).toContain("REQ_DEPTH_FRAME = 8")
    expect(engine).toContain("run_depth_frame")
    expect(loader).toContain("image->storage = AlignedBytes(checked_align(total))")
    expect(weights).toContain("Tensor::from_raw_buffer(view.data()")
    expect(deletion).toContain("It is currently a thin re-export")
    expect(runbook).toContain("The native model SQ/CQ is internal to the native runtime")
    expect(runbook).toContain("Rust does not broker")
    expect(runbook).toContain("model passes.")
    expect(runbook).toContain("The completed production source graph has no numerical Rust or C++ body")
    expect(runbook).toContain("It imports no libm numerical symbol")
    expect(map).toContain("It does not broker Flashkern")
    expect(cpu).toContain("the native conversation submits and recurs full passes")
    expect(cpu).toContain("AS-BUILT: one immutable combined image")
    expect(design).toContain("Rust is not step 4 or step 7")
    expect(design).toContain("no C++ production numerical expressions")
    expect(kernels).toContain("# Flashkern CPU Assembly Contract")
    expect(kernels).toContain("## C++ Numerical Debt")
    expect(kernels).toContain("expected-value generation fence")
    expect(kernels).toContain("| threadgroup barrier | expected-value generation fence |")
    expect(kernels).toContain("Flashkern is CPU-only")
    expect(voice).toContain("Rust owns audio streams in and out")
    expect(voice).toContain("Rust kcoro owns asynchronous I/O continuations")
    expect(voice).toContain("Assembly owns every numerical operation")
    expect(voice).toContain("submit_pass` no longer calls a Rust coordinator")
    expect(voice).toContain("Numerical C++ remains")
    expect(tickets).toContain("### Target Orchestration Flow")
    expect(tickets).toContain("Native session control owns the mutable model ticket table")
    expect(tickets).not.toContain("Rust kcoro owns the mutable ticket table")
  })
  ...
  test("utterance workers invalidate queued stale audio on interrupt", async () => {
    ...
    expect(local).toContain("Some(pipe.submit_pcm(&mic_buf[start..end], in_rate))")
  ...
  test("native LiveKit turn mic ingestion uses assistant playback as VAD reference", async () => {
    ...
    expect(events).toContain("playback.observe(&pcm, rate)")
  ...
  test("desktop model loading is settings-driven, never process-environment configured", async () => {
    const loader = await root("crates/liquid-audio/src/loader.rs")
    ...
    const runtime = await root("packages/desktop/src-tauri/src/voice/runtime.rs")
    const settings = await root("packages/desktop/src-tauri/src/settings.rs")
    const panel = await root("packages/app/src/components/settings-voice.tsx")
    ...
    expect(runtime).toContain("settings::lfm2_active_model_dir(&settings.lfm2)")
    expect(runtime).toContain("resident_lfm2(&dir, &settings.lfm2.device)")
