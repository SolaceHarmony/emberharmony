...
describe("desktop voice context boundary", () => {
  ...
  test("native audio callbacks publish into bounded capture reservations", async () => {
    const text = await liquid("src/runtime/voice_runtime.rs")

    expect(text).toContain("struct CaptureIngress")
    expect(text).toContain("blocks: [Arc<CaptureReservation>; CAPTURE_BLOCKS]")
    expect(text).toContain("active: AtomicUsize")
    expect(text).toContain("notify: RealtimeNotifier")
    expect(text).toContain("let _ = self.notify.notify()")
    expect(text).not.toContain("Doorbell")
    expect(text).not.toContain(".park(")
    expect(text).not.toContain("wake_tx: Sender<()>")
    expect(text).not.toContain("wake_rx: Receiver<()>")
  ...
  test("native mic continuations resume from producer edges without operation waiters", async () => {
    const text = await liquid("src/runtime/voice_runtime.rs")
    const vad = between(text, "impl VadTask", "impl Drop for VadTask")
    const input = between(text, "fn start_input(", "fn start_output")

    expect(vad).toContain("silent_frames >= self.silence_frames")
    expect(vad).toContain("ServiceOutcome::Dormant")
    expect(input).toContain("dev.build_input_stream(")
    expect(input).toContain("AudioInputWriter::push_interleaved_f32")
    expect(input).toContain("AudioInputWriter::push_interleaved_i16")
    expect(input).toContain("AudioInputWriter::push_interleaved_u16")
    expect(vad).not.toContain(".park(")
  ...
  test("native control commands resume the retained mic continuation immediately", async () => {
    ...
    expect(runtime).toContain("let _ = self.control.notify()")
  ...
  test("desktop LFM2 uses liquid-audio direct platform callbacks", async () => {
    ...
    expect(audio).toContain("start_input(writer, init.in_rate)")
  ...
  test("native LFM2 VAD treats speaker playback as reference audio for barge-in", async () => {
    ...
    expect(runtime).toContain("reference_audio_active(&self.assistant, &playback)")
    expect(runtime).toContain("reference_vad_threshold(")
    expect(runtime).toContain("self.cfg.vad_threshold")
    expect(runtime).toContain("assistant.load(Ordering::SeqCst) || playback.active_or_tail()")
  ...
  test("desktop LFM2 construction opens only the opaque native model", async () => {
    ...
    expect(build).toContain("let engine: NativeLfm2VoiceEngine = model.engine(sampling, vault, out_rate)?")
  ...
  test("desktop voice docs describe the implemented Tauri kernel command surface", async () => {
    ...
    expect(architecture).toContain("Rust owns audio streams in and out")
    expect(architecture).toContain("Rust kcoro owns asynchronous I/O continuations")
