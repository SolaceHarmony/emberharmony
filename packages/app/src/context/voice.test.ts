...
describe("desktop voice context boundary", () => {
  ...
  test("kcoro documentation separates the mounted pass edge from target orchestration", async () => {
    ...
    expect(decode).toContain("pub(crate) struct SpinBarrier")
    ...
    expect(mounted).toContain("transitional `REQ_CALL` programs such as `DepthDecode`")
    ...
    expect(kernels).toContain("Rust `SpinBarrier` polls between active stages")
    ...
    expect(tickets).toContain("sampling and recurrence still occur in the Rust/Candle caller")
  })
