...
describe("desktop voice context boundary", () => {
  ...
  test("kcoro SQ/CQ bridge is wired into production decode and test contracts", async () => {
    ...
    const coordinator = await liquid("src/compute/flashkern/coordinator.rs")
    const ops = await liquid("src/compute/flashkern/candle_ops.rs")
    ...
    const cpuRim = between(ops, "pub fn depthwise_conv1d_stream(", "#[cfg(test)]")

    expect(cargo).toContain('kcoro-sys = { path = "../kcoro-sys" }')
    expect(cargo).toContain('kcoro = { path = "../kcoro" }')
    expect(cargo).toContain('candle-flashfftconv = { path = "../candle-flashfftconv", optional = true }')
    ...
    expect(native).toContain("Coordinator::new")
    expect(native).toContain("lfm_engine_set_submitter")
    expect(native).toContain("pub fn process_engine()")
    expect(coordinator).toContain("lfm_kernel_bridge_submit")
    expect(coordinator).toContain("lfm_kernel_bridge_wait_completion")
    expect(coordinator).toContain("receiver.recv().await")
    expect(coordinator).not.toContain("tauri")
    expect(coordinator).not.toContain("tokio")
    expect(engine).toContain("e->submitter")
    expect(engine).not.toContain("lfm_kernel_bridge_submit")
    expect(engine).not.toContain("lfm_kernel_bridge_wait_completion")
    expect(engine).toContain("lfm_kernel_bridge_wait_submission")
    expect(engine).toContain("lfm_kernel_bridge_publish_completion")
    expect(engine).not.toContain("kc_ticket_")
    expect(engine).not.toContain("kc_runtime_")
    ...
    expect(model).toContain("Some(&text_sampler)")
    expect(model).toContain("Some(&mut sampling)")
    expect(model).toContain("sample_audio_frame_flash")
    ...
    expect(backbone).toContain("if bx.device().is_cpu()")
    expect(backbone).toContain("candle_flashfftconv::depthwise_conv1d_stream")
    expect(cpuRim).toContain("Flashkern depthwise stream requires CPU bf16")
    expect(cpuRim).not.toContain("candle_flashfftconv")
    expect(cpuRim).not.toContain('feature = "metal"')
    expect(ci).toContain("cargo test -p kcoro-sys --tests -- --nocapture")
  })

  test("kcoro documentation separates the mounted pass edge from target orchestration", async () => {
    const native = await liquid("src/compute/flashkern/native_engine.rs")
    const coordinator = await liquid("src/compute/flashkern/coordinator.rs")
    const decode = await liquid("src/compute/flashkern/decode.rs")
    ...
    const queue = engine.match(/\.capacity = (\d+),/)
    const descriptors = engine.match(/\.descriptor_capacity = (\d+),/)
    const results = native.match(/Coordinator::new\(bridge, (\d+)\)/)
    ...
    expect(coordinator).toContain("ready: Condvar::new()")
    expect(coordinator).toContain("pending: Mutex::new(None)")
    expect(decode).toContain("pub struct DepthDecode")
    expect(decode).not.toContain("pub(crate) struct SpinBarrier")
    ...
    expect(mounted).toContain("sequenceDiagram")
    expect(mounted).toContain(`| Native SQ/CQ capacity | \`${queue?.[1]}\``)
    expect(mounted).toContain(`| Native descriptor slots | \`${descriptors?.[1]}\``)
    expect(mounted).toContain(`| Rust result/ring slots | \`${results?.[1]}\``)
    expect(mounted).toContain("payload = Engine*")
    expect(mounted).toContain("still synchronous at the outer C++/Rust rim")
    expect(runbook).toContain("`REQ_DEPTH_FRAME` is the typed native Depthformer program")
    expect(mounted).not.toContain("transitional `REQ_CALL` programs such as `DepthDecode`")
    expect(mounted).not.toContain("Parent Rust future")
    expect(map).toContain("`crates/kcoro` - dependency-free safe Rust coordination kernel")
    expect(map).toContain("Its C ticket scheduler is tested but is")
    expect(cpu).toContain("safe Rust `kcoro::Executor` owns one broker future")
    expect(cpu).toContain("AS-BUILT: native image plus counted compatibility copies")
    expect(cpu).not.toContain("kcoro_arena owns stackless coordination")
    expect(design).toContain("The sibling `kcoro-sys` crate")
    expect(design).toContain("## 6. Candle disposition")
    expect(design).not.toContain("the safetensors mmap IS the weight buffer")
    expect(kernels).toContain("`fused_mlp_reference` function in `decode.rs` is test-only")
    expect(kernels).toContain("zero-spin generation fences")
    expect(kernels).toContain("threadgroup-equivalent stage fence")
    expect(kernels).toContain("**Backend boundary:** Flashkern is CPU-only")
    expect(design).toContain("Flashkern never owns Metal dispatch")
    expect(cpu).toContain("Flashkern is the CPU executor, never the Metal executor")
    expect(runbook).toContain("Flashkern never owns Metal dispatch")
    expect(voice).toContain("Rust kcoro\\nSQ broker + CQ ingress")
    expect(voice).toContain("The outer call remains synchronous")
    expect(runbook).toContain("## Target Broker And Ticket Flow")
    expect(runbook.indexOf("### Mounted Pass Sequence (`4f06a3d5`)")).toBeLessThan(
      runbook.indexOf("## Target Broker And Ticket Flow"),
    )
    expect(tickets).toContain("### Target Orchestration Flow")
    expect(tickets).toContain("not the foundation's exact-once `Promise`")
    expect(tickets).toContain("sampling occurs inside the")
    expect(tickets).toContain("typed native token and Depthformer passes")
  })
