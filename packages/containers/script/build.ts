...
const reg = process.env.REGISTRY ?? "ghcr.io/solaceharmony"
...
const setup = async () => {
  ...
  if (list.includes("code-harmony")) {
    await $`docker buildx use code-harmony`
    return
  }
  await $`docker buildx create --name code-harmony --use`
}
