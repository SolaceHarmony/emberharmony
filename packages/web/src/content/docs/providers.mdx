---
title: Providers
description: Using any LLM provider in CodeHarmony.
---

import config from "../../../config.mjs"
export const console = config.console

CodeHarmony uses the [AI SDK](https://ai-sdk.dev/) and [Models.dev](https://models.dev) to support **75+ LLM providers** and it supports running local models.

To add a provider you need to:

1. Add the API keys for the provider using the `/connect` command.
2. Configure the provider in your CodeHarmony config.
...
in `~/.local/share/code-harmony/auth.json`.

---

### Config

You can customize the providers through the `provider` section in your CodeHarmony
config.
...
```json title="code-harmony.json" {6}
{
...
CodeHarmony Zen is a list of models provided by the CodeHarmony team that have been
tested and verified to work well with CodeHarmony. [Learn more](/docs/zen).

:::tip
If you are new, we recommend starting with CodeHarmony Zen.
:::

1. Run the `/connect` command in the TUI, select code-harmony, and head to [solace.ofharmony.ai/auth](https://solace.ofharmony.ai/auth).
...
It works like any other provider in CodeHarmony and is completely optional to use.
...
To use Amazon Bedrock with CodeHarmony:
...
2. **Configure authentication** using one of the following methods:
   ...
   Set one of these environment variables while running code-harmony:

   ```bash
   # Option 1: Using AWS access keys
   AWS_ACCESS_KEY_ID=XXX AWS_SECRET_ACCESS_KEY=YYY code-harmony

   # Option 2: Using named AWS profile
   AWS_PROFILE=my-profile code-harmony

   # Option 3: Using Bedrock bearer token
   AWS_BEARER_TOKEN_BEDROCK=XXX code-harmony
   ```
   ...
   For project-specific or persistent configuration, use `code-harmony.json`:

   ```json title="code-harmony.json"
   {
     "$schema": "https://solace.ofharmony.ai/config.json",
     "provider": {
       "amazon-bedrock": {
...
:::info
Using your Claude Pro/Max subscription in CodeHarmony is not officially supported by [Anthropic](https://anthropic.com).
:::
...
2. Go to [Azure AI Foundry](https://ai.azure.com/) and deploy a model.
   ...
   :::note
   The deployment name must match the model name for code-harmony to work properly.
   :::
...
5. Set your resource name as an environment variable:
   ...
   ```bash
   AZURE_RESOURCE_NAME=XXX code-harmony
   ```
...
5. Set your resource name as an environment variable:
   ...
   ```bash
   AZURE_COGNITIVE_SERVICES_RESOURCE_NAME=XXX code-harmony
   ```
...
5. Run the `/models` command to select a model.
   ...
   You can also add models through your code-harmony config.
...
to store token in code-harmony auth storage.
:::

##### Self-Hosted GitLab

:::note[compliance note]
CodeHarmony uses a small model for some AI tasks like generating the session title.
It is configured to use gpt-5-nano by default, hosted by Zen. To lock CodeHarmony
to only use your own GitLab-hosted instance, add the following to your
`code-harmony.json` file. It is also recommended to disable session sharing.
...
More documentation on [code-harmony-gitlab-auth](https://www.npmjs.com/package/@gitlab/code-harmony-gitlab-auth) homepage.

##### Configuration

Customize through `code-harmony.json`:
...
{
  "$schema": "https://solace.ofharmony.ai/config.json",
  "plugin": ["@gitlab/code-harmony-gitlab-plugin"]
}
```
...
To use your GitHub Copilot subscription with code-harmony:
...
To use Google Vertex AI with CodeHarmony:
...
2. Set the required environment variables:
   ...
   Set them while running code-harmony.

   ```bash
   GOOGLE_APPLICATION_CREDENTIALS=/path/to/service-account.json GOOGLE_CLOUD_PROJECT=your-project-id code-harmony
   ```
...
In the event you see a feature or model from Helicone that isn't configured automatically through code-harmony, you can always configure it yourself.
...
```jsonc title="~/.config/code-harmony/code-harmony.jsonc"
{
  "$schema": "https://solace.ofharmony.ai/config.json",
  "provider": {
    "helicone": {
...
Helicone's [Sessions](https://docs.helicone.ai/features/sessions) feature lets you group related LLM requests together. Use the [code-harmony-helicone-session](https://github.com/H2Shami/code-harmony-helicone-session) plugin to automatically log each CodeHarmony conversation as a session in Helicone.

```bash
npm install -g code-harmony-helicone-session
```
...
```json title="code-harmony.json"
{
  "plugin": ["code-harmony-helicone-session"]
}
```

The plugin injects `Helicone-Session-Id` and `Helicone-Session-Name` headers into your requests. In Helicone's Sessions page, you'll see each CodeHarmony conversation listed as a separate session.
...
You can configure code-harmony to use local models through [llama.cpp's](https://github.com/ggml-org/llama.cpp) llama-server utility

```json title="code-harmony.json" "llama.cpp" {5, 6, 8, 10-15}
{
  "$schema": "https://solace.ofharmony.ai/config.json",
  "provider": {
    "llama.cpp": {
...
You can configure code-harmony to use local models through LM Studio.

```json title="code-harmony.json" "lmstudio" {5, 6, 8, 10-14}
{
  "$schema": "https://solace.ofharmony.ai/config.json",
  "provider": {
    "lmstudio": {
...
You can configure code-harmony to use local models through Ollama.

```json title="code-harmony.json" "ollama" {5, 6, 8, 10-14}
{
  "$schema": "https://solace.ofharmony.ai/config.json",
  "provider": {
    "ollama": {
...
To use Ollama Cloud with CodeHarmony:
...
3. Copy the API key for use in CodeHarmony.
...
6. **Important**: Before using cloud models in CodeHarmony, you must pull the model information locally:
...
CodeHarmony Zen is a list of tested and verified models provided by the CodeHarmony team. [Learn more](/docs/zen).

1. Sign in to **<a href={console}>CodeHarmony Zen</a>** and click **Create API Key**.

2. Run the `/connect` command and search for **CodeHarmony Zen**.

   ```txt
   /connect
   ```

3. Enter your CodeHarmony API key.
...
4. Many OpenRouter models are preloaded by default, run the `/models` command to select the one you want.
   ...
   You can also add additional models through your code-harmony config.
...
5. You can also customize them through your code-harmony config. Here's an example of specifying a provider
...
3. Enter your service key JSON.
   ...
   ```bash
   AICORE_SERVICE_KEY='{"clientid":"...","clientsecret":"...","url":"...","serviceurls":{"AI_API_URL":"..."}}' code-harmony
   ```
...
4. Optionally set deployment ID and resource group:
   ...
   ```bash
   AICORE_DEPLOYMENT_ID=your-deployment-id AICORE_RESOURCE_GROUP=your-resource-group code-harmony
   ```
...
To use [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) with CodeHarmony:
...
You can also customize models through your code-harmony config. Here's an example of specifying provider routing order.
...
:::tip
You can use any OpenAI-compatible provider with code-harmony. Most modern AI providers offer OpenAI-compatible APIs.
:::
...
3. Enter your API key for the provider.
   ...
   ┌  Add credential
   │
   ▲  This only stores a credential for myprovider - you will need to configure it in code-harmony.json, check the docs for examples.
   │
   ◇  Enter your API key
   │  sk-...
   └
...
4. Create or update your `code-harmony.json` file in your project directory:

   ```json title="code-harmony.json" ""myprovider"" {5-15}
   {
     "$schema": "https://solace.ofharmony.ai/config.json",
     "provider": {
       "myprovider": {
...
```json title="code-harmony.json" {9,11,17-20}
{
  "$schema": "https://solace.ofharmony.ai/config.json",
  "provider": {
    "myprovider": {
...
The `limit` fields allow CodeHarmony to understand how much context you have left. Standard providers pull these from models.dev automatically.
...
1. **Check the auth setup**: Run `code-harmony auth list` to see if the credentials
...
2. For custom providers, check the code-harmony config and:
   - Make sure the provider ID used in the `/connect` command matches the ID in your code-harmony config.
