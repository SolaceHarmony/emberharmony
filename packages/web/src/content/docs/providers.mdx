---
title: Providers
description: Using any LLM provider in EmberHarmony.
---

import config from "../../../config.mjs"
export const console = config.console

EmberHarmony uses the [AI SDK](https://ai-sdk.dev/) and [Models.dev](https://models.dev) to support **75+ LLM providers** and it supports running local models.

To add a provider you need to:

1. Add the API keys for the provider using the `/connect` command.
2. Configure the provider in your EmberHarmony config.
...
in `~/.local/share/emberharmony/auth.json`.

---

### Config

You can customize the providers through the `provider` section in your EmberHarmony
config.
...
```json title="emberharmony.json" {6}
{
...
EmberHarmony Zen is a list of models provided by the EmberHarmony team that have been
tested and verified to work well with EmberHarmony. [Learn more](/docs/zen).

:::tip
If you are new, we recommend starting with EmberHarmony Zen.
:::
...
It works like any other provider in EmberHarmony and is completely optional to use.
...
To use Amazon Bedrock with EmberHarmony:
...
2. **Configure authentication** using one of the following methods:
   ...
   Set one of these environment variables while running emberharmony:

   ```bash
   # Option 1: Using AWS access keys
   AWS_ACCESS_KEY_ID=XXX AWS_SECRET_ACCESS_KEY=YYY emberharmony

   # Option 2: Using named AWS profile
   AWS_PROFILE=my-profile emberharmony

   # Option 3: Using Bedrock bearer token
   AWS_BEARER_TOKEN_BEDROCK=XXX emberharmony
   ```
   ...
   For project-specific or persistent configuration, use `emberharmony.json`:

   ```json title="emberharmony.json"
   {
     "$schema": "https://solace.ofharmony.ai/config.json",
     "provider": {
       "amazon-bedrock": {
...
:::info
Using your Claude Pro/Max subscription in EmberHarmony is not officially supported by [Anthropic](https://anthropic.com).
:::
...
2. Go to [Azure AI Foundry](https://ai.azure.com/) and deploy a model.
   ...
   :::note
   The deployment name must match the model name for emberharmony to work properly.
   :::
...
5. Set your resource name as an environment variable:
   ...
   ```bash
   AZURE_RESOURCE_NAME=XXX emberharmony
   ```
...
5. Set your resource name as an environment variable:
   ...
   ```bash
   AZURE_COGNITIVE_SERVICES_RESOURCE_NAME=XXX emberharmony
   ```
...
5. Run the `/models` command to select a model.
   ...
   You can also add models through your emberharmony config.
...
2. Choose your authentication method:
   ...
   3. Name: `EmberHarmony`, Scopes: `api`
...
to store token in emberharmony auth storage.
:::

##### Self-Hosted GitLab

:::note[compliance note]
EmberHarmony uses a small model for some AI tasks like generating the session title.
It is configured to use gpt-5-nano by default, hosted by Zen. To lock EmberHarmony
to only use your own GitLab-hosted instance, add the following to your
`emberharmony.json` file. It is also recommended to disable session sharing.
...
More documentation on [emberharmony-gitlab-auth](https://www.npmjs.com/package/@gitlab/emberharmony-gitlab-auth) homepage.

##### Configuration

Customize through `emberharmony.json`:
...
{
  "$schema": "https://solace.ofharmony.ai/config.json",
  "plugin": ["@gitlab/emberharmony-gitlab-plugin"]
}
```
...
To use your GitHub Copilot subscription with emberharmony:
...
To use Google Vertex AI with EmberHarmony:
...
2. Set the required environment variables:
   ...
   Set them while running emberharmony.

   ```bash
   GOOGLE_APPLICATION_CREDENTIALS=/path/to/service-account.json GOOGLE_CLOUD_PROJECT=your-project-id emberharmony
   ```
...
In the event you see a feature or model from Helicone that isn't configured automatically through emberharmony, you can always configure it yourself.
...
```jsonc title="~/.config/emberharmony/emberharmony.jsonc"
{
  "$schema": "https://solace.ofharmony.ai/config.json",
  "provider": {
    "helicone": {
...
Helicone's [Sessions](https://docs.helicone.ai/features/sessions) feature lets you group related LLM requests together. Use the [emberharmony-helicone-session](https://github.com/H2Shami/emberharmony-helicone-session) plugin to automatically log each EmberHarmony conversation as a session in Helicone.

```bash
npm install -g emberharmony-helicone-session
```
...
```json title="emberharmony.json"
{
  "plugin": ["emberharmony-helicone-session"]
}
```

The plugin injects `Helicone-Session-Id` and `Helicone-Session-Name` headers into your requests. In Helicone's Sessions page, you'll see each EmberHarmony conversation listed as a separate session.
...
You can configure emberharmony to use local models through [llama.cpp's](https://github.com/ggml-org/llama.cpp) llama-server utility

```json title="emberharmony.json" "llama.cpp" {5, 6, 8, 10-15}
{
  "$schema": "https://solace.ofharmony.ai/config.json",
  "provider": {
    "llama.cpp": {
...
You can configure emberharmony to use local models through LM Studio.

```json title="emberharmony.json" "lmstudio" {5, 6, 8, 10-14}
{
  "$schema": "https://solace.ofharmony.ai/config.json",
  "provider": {
    "lmstudio": {
...
You can configure emberharmony to use local models through Ollama.

```json title="emberharmony.json" "ollama" {5, 6, 8, 10-14}
{
  "$schema": "https://solace.ofharmony.ai/config.json",
  "provider": {
    "ollama": {
...
To use Ollama Cloud with EmberHarmony:
...
3. Copy the API key for use in EmberHarmony.
...
6. **Important**: Before using cloud models in EmberHarmony, you must pull the model information locally:
...
EmberHarmony Zen is a list of tested and verified models provided by the EmberHarmony team. [Learn more](/docs/zen).

1. Sign in to **<a href={console}>EmberHarmony Zen</a>** and click **Create API Key**.

2. Run the `/connect` command and search for **EmberHarmony Zen**.

   ```txt
   /connect
   ```

3. Enter your EmberHarmony API key.
...
4. Many OpenRouter models are preloaded by default, run the `/models` command to select the one you want.
   ...
   You can also add additional models through your emberharmony config.
...
5. You can also customize them through your emberharmony config. Here's an example of specifying a provider
...
3. Enter your service key JSON.
   ...
   ```bash
   AICORE_SERVICE_KEY='{"clientid":"...","clientsecret":"...","url":"...","serviceurls":{"AI_API_URL":"..."}}' emberharmony
   ```
...
4. Optionally set deployment ID and resource group:
   ...
   ```bash
   AICORE_DEPLOYMENT_ID=your-deployment-id AICORE_RESOURCE_GROUP=your-resource-group emberharmony
   ```
...
To use [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) with EmberHarmony:
...
You can also customize models through your emberharmony config. Here's an example of specifying provider routing order.
...
:::tip
You can use any OpenAI-compatible provider with emberharmony. Most modern AI providers offer OpenAI-compatible APIs.
:::
...
3. Enter your API key for the provider.
   ...
   ┌  Add credential
   │
   ▲  This only stores a credential for myprovider - you will need to configure it in emberharmony.json, check the docs for examples.
   │
   ◇  Enter your API key
   │  sk-...
   └
...
4. Create or update your `emberharmony.json` file in your project directory:

   ```json title="emberharmony.json" ""myprovider"" {5-15}
   {
     "$schema": "https://solace.ofharmony.ai/config.json",
     "provider": {
       "myprovider": {
...
```json title="emberharmony.json" {9,11,17-20}
{
  "$schema": "https://solace.ofharmony.ai/config.json",
  "provider": {
    "myprovider": {
...
The `limit` fields allow EmberHarmony to understand how much context you have left. Standard providers pull these from models.dev automatically.
...
1. **Check the auth setup**: Run `emberharmony auth list` to see if the credentials
...
2. For custom providers, check the emberharmony config and:
   - Make sure the provider ID used in the `/connect` command matches the ID in your emberharmony config.
