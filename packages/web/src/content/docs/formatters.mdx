---
title: Formatters
description: CodeHarmony uses language specific formatters.
---

CodeHarmony automatically formats files after they are written or edited using language-specific formatters. This ensures that the code that is generated follows the code styles of your project.

---

## Built-in

CodeHarmony comes with several built-in formatters for popular languages and frameworks. Below is a list of the formatters, supported file extensions, and commands or config options it needs.
...
So if your project has `prettier` in your `package.json`, CodeHarmony will automatically use it.

---

## How it works

When CodeHarmony writes or edits a file, it:
...
You can customize formatters through the `formatter` section in your CodeHarmony config.
...
```json title="code-harmony.json" {5}
{
  "$schema": "https://solace.ofharmony.ai/config.json",
  "formatter": {
    "prettier": {
