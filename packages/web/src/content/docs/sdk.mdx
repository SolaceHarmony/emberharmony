---
title: SDK
description: Type-safe JS client for emberharmony server.
---

import config from "../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

The emberharmony JS/TS SDK provides a type-safe client for interacting with the server.
Use it to build integrations and control emberharmony programmatically.
...
```bash
npm install @thesolaceproject/emberharmony-sdk
```
...
Create an instance of emberharmony:

```javascript
import { createEmberHarmony } from "@thesolaceproject/emberharmony-sdk"

const { client } = await createEmberHarmony()
```
...
You can pass a configuration object to customize behavior. The instance still picks up your `emberharmony.json`, but you can override or add configuration inline:

```javascript
import { createEmberHarmony } from "@thesolaceproject/emberharmony-sdk"

const emberharmony = await createEmberHarmony({
  hostname: "127.0.0.1",
  port: 4096,
  config: {
...
console.log(`Server running at ${emberharmony.server.url}`)

emberharmony.server.close()
```

## Client only

If you already have a running instance of emberharmony, you can create a client instance to connect to it:
...
```typescript
import type { Session, Message, Part } from "@thesolaceproject/emberharmony-sdk"
```
...
const textResults = await client.find.text({
  query: { pattern: "function.*emberharmony" },
})
