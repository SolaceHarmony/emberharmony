---
title: SDK
description: Type-safe JS client for code-harmony server.
---

import config from "../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

The code-harmony JS/TS SDK provides a type-safe client for interacting with the server.
Use it to build integrations and control code-harmony programmatically.
...
```bash
npm install @thesolaceproject/code-harmony-sdk
```
...
Create an instance of code-harmony:

```javascript
import { createCodeHarmony } from "@thesolaceproject/code-harmony-sdk"

const { client } = await createCodeHarmony()
```
...
You can pass a configuration object to customize behavior. The instance still picks up your `code-harmony.json`, but you can override or add configuration inline:

```javascript
import { createCodeHarmony } from "@thesolaceproject/code-harmony-sdk"

const code-harmony = await createCodeHarmony({
  hostname: "127.0.0.1",
  port: 4096,
  config: {
...
console.log(`Server running at ${code-harmony.server.url}`)

code-harmony.server.close()
```

## Client only

If you already have a running instance of code-harmony, you can create a client instance to connect to it:
...
```typescript
import type { Session, Message, Part } from "@thesolaceproject/code-harmony-sdk"
```
...
const textResults = await client.find.text({
  query: { pattern: "function.*code-harmony" },
})
