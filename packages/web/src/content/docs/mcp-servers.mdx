...
You can add external tools to EmberHarmony using the _Model Context Protocol_, or MCP. EmberHarmony supports both local and remote servers.
...
You can define MCP servers in your [EmberHarmony Config](https://solace.ofharmony.ai/docs/config/) under `mcp`. Add each MCP with a unique name. You can refer to that MCP by name when prompting the LLM.

```jsonc title="emberharmony.jsonc" {6}
{
...
Organizations can provide default MCP servers via their `.well-known/emberharmony` endpoint. These servers may be disabled by default, allowing users to opt-in to the ones they need.
...
```jsonc title="emberharmony.jsonc" {15}
{
  "$schema": "https://solace.ofharmony.ai/config.json",
  "mcp": {
    "my-local-mcp-server": {
...
EmberHarmony automatically handles OAuth authentication for remote MCP servers. When a server requires authentication, EmberHarmony will:
...
If the server requires authentication, EmberHarmony will prompt you to authenticate when you first try to use it. If not, you can [manually trigger the flow](#authenticating) with `emberharmony mcp auth <server-name>`.
...
```json title="emberharmony.json" {7-11}
{
  "$schema": "https://solace.ofharmony.ai/config.json",
  "mcp": {
    "my-oauth-server": {
...
```bash
emberharmony mcp auth my-oauth-server
```
...
```bash
emberharmony mcp logout my-oauth-server
```

The `mcp auth` command will open your browser for authorization. After you authorize, EmberHarmony will store the tokens securely in `~/.local/share/emberharmony/mcp-auth.json`.
...
```json title="emberharmony.json" {7}
{
  "$schema": "https://solace.ofharmony.ai/config.json",
  "mcp": {
    "my-api-key-server": {
...
emberharmony mcp debug my-oauth-server
```
...
Your MCPs are available as tools in EmberHarmony, alongside built-in tools. So you can manage them through the EmberHarmony config like any other tool.
...
```json title="emberharmony.json" {14}
{
  "$schema": "https://solace.ofharmony.ai/config.json",
  "mcp": {
    "my-mcp-foo": {
...
```json title="emberharmony.json" {11, 14-18}
{
  "$schema": "https://solace.ofharmony.ai/config.json",
  "mcp": {
    "my-mcp": {
...
```json title="emberharmony.json" {4-8}
{
  "$schema": "https://solace.ofharmony.ai/config.json",
  "mcp": {
    "sentry": {
...
```bash
emberharmony mcp auth sentry
```

This will open a browser window to complete the OAuth flow and connect EmberHarmony to your Sentry account.
...
```json title="emberharmony.json" {4-7}
{
  "$schema": "https://solace.ofharmony.ai/config.json",
  "mcp": {
    "context7": {
...
```json title="emberharmony.json" {7-9}
{
  "$schema": "https://solace.ofharmony.ai/config.json",
  "mcp": {
    "context7": {
...
What's the right way to set a custom domain in an Astro component? use the gh_grep tool
```
