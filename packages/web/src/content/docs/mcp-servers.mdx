...
You can add external tools to CodeHarmony using the _Model Context Protocol_, or MCP. CodeHarmony supports both local and remote servers.
...
You can define MCP servers in your [CodeHarmony Config](https://solace.ofharmony.ai/docs/config/) under `mcp`. Add each MCP with a unique name. You can refer to that MCP by name when prompting the LLM.

```jsonc title="code-harmony.jsonc" {6}
{
...
Organizations can provide default MCP servers via their `.well-known/code-harmony` endpoint. These servers may be disabled by default, allowing users to opt-in to the ones they need.
...
```jsonc title="code-harmony.jsonc" {15}
{
  "$schema": "https://solace.ofharmony.ai/config.json",
  "mcp": {
    "my-local-mcp-server": {
...
CodeHarmony automatically handles OAuth authentication for remote MCP servers. When a server requires authentication, CodeHarmony will:
...
If the server requires authentication, CodeHarmony will prompt you to authenticate when you first try to use it. If not, you can [manually trigger the flow](#authenticating) with `code-harmony mcp auth <server-name>`.
...
```json title="code-harmony.json" {7-11}
{
  "$schema": "https://solace.ofharmony.ai/config.json",
  "mcp": {
    "my-oauth-server": {
...
```bash
code-harmony mcp auth my-oauth-server
```
...
```bash
code-harmony mcp logout my-oauth-server
```

The `mcp auth` command will open your browser for authorization. After you authorize, CodeHarmony will store the tokens securely in `~/.local/share/code-harmony/mcp-auth.json`.
...
```json title="code-harmony.json" {7}
{
  "$schema": "https://solace.ofharmony.ai/config.json",
  "mcp": {
    "my-api-key-server": {
...
code-harmony mcp debug my-oauth-server
```
...
Your MCPs are available as tools in CodeHarmony, alongside built-in tools. So you can manage them through the CodeHarmony config like any other tool.
...
```json title="code-harmony.json" {14}
{
  "$schema": "https://solace.ofharmony.ai/config.json",
  "mcp": {
    "my-mcp-foo": {
...
```json title="code-harmony.json" {11, 14-18}
{
  "$schema": "https://solace.ofharmony.ai/config.json",
  "mcp": {
    "my-mcp": {
...
```json title="code-harmony.json" {4-8}
{
  "$schema": "https://solace.ofharmony.ai/config.json",
  "mcp": {
    "sentry": {
...
```bash
code-harmony mcp auth sentry
```

This will open a browser window to complete the OAuth flow and connect CodeHarmony to your Sentry account.
...
```json title="code-harmony.json" {4-7}
{
  "$schema": "https://solace.ofharmony.ai/config.json",
  "mcp": {
    "context7": {
...
```json title="code-harmony.json" {7-9}
{
  "$schema": "https://solace.ofharmony.ai/config.json",
  "mcp": {
    "context7": {
