...
:::caution
Modes are now configured through the `agent` option in the code-harmony config. The
`mode` option is now deprecated. [Learn more](/docs/agents).
:::

Modes in code-harmony allow you to customize the behavior, tools, and prompts for different use cases.

It comes with two built-in modes: **build** and **plan**. You can customize
these or configure your own through the code-harmony config.
...
code-harmony comes with two built-in modes.
...
- `edit` - Cannot modify existing files, except for files located at `.code-harmony/plans/*.md` to detail the plan itself
...
Configure modes in your `code-harmony.json` config file:
...
- Global: `~/.config/code-harmony/modes/`
- Project: `.code-harmony/modes/`

```markdown title="~/.config/code-harmony/modes/review.md"
---
model: anthropic/claude-sonnet-4-20250514
temperature: 0.1
tools:
...
If no temperature is specified, code-harmony uses model-specific defaults (typically 0 for most models, 0.55 for Qwen models).
...
both the global code-harmony config and the project specific config.
...
```json title="code-harmony.json" {4-14}
{
  "$schema": "https://solace.ofharmony.ai/config.json",
  "mode": {
    "docs": {
...
Create mode files in `.code-harmony/modes/` for project-specific modes or `~/.config/code-harmony/modes/` for global modes:

```markdown title=".code-harmony/modes/debug.md"
---
temperature: 0.1
tools:
  bash: true
...
```markdown title="~/.config/code-harmony/modes/refactor.md"
---
model: anthropic/claude-sonnet-4-20250514
temperature: 0.2
tools:
