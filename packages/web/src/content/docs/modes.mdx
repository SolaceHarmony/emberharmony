...
:::caution
Modes are now configured through the `agent` option in the emberharmony config. The
`mode` option is now deprecated. [Learn more](/docs/agents).
:::

Modes in emberharmony allow you to customize the behavior, tools, and prompts for different use cases.

It comes with two built-in modes: **build** and **plan**. You can customize
these or configure your own through the emberharmony config.
...
emberharmony comes with two built-in modes.
...
- `edit` - Cannot modify existing files, except for files located at `.emberharmony/plans/*.md` to detail the plan itself
...
Configure modes in your `emberharmony.json` config file:
...
- Global: `~/.config/emberharmony/modes/`
- Project: `.emberharmony/modes/`

```markdown title="~/.config/emberharmony/modes/review.md"
---
model: anthropic/claude-sonnet-4-20250514
temperature: 0.1
tools:
...
both the global emberharmony config and the project specific config.
...
```json title="emberharmony.json" {4-14}
{
  "$schema": "https://solace.ofharmony.ai/config.json",
  "mode": {
    "docs": {
...
Create mode files in `.emberharmony/modes/` for project-specific modes or `~/.config/emberharmony/modes/` for global modes:

```markdown title=".emberharmony/modes/debug.md"
---
temperature: 0.1
tools:
  bash: true
...
```markdown title="~/.config/emberharmony/modes/refactor.md"
---
model: anthropic/claude-sonnet-4-20250514
temperature: 0.2
tools:
