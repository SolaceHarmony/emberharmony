...
Set `OPENCODE_SERVER_PASSWORD` to protect the server with HTTP basic auth. The username defaults to `code-harmony`, or set `OPENCODE_SERVER_USERNAME` to override it. This applies to both `code-harmony serve` and `code-harmony web`.

```bash
OPENCODE_SERVER_PASSWORD=your-password code-harmony serve
```
