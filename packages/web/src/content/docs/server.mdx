---
title: Server
description: Interact with code-harmony server over HTTP.
---

import config from "../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

The `code-harmony serve` command runs a headless HTTP server that exposes an OpenAPI endpoint that an code-harmony client can use.
...
```bash
code-harmony serve [--port <number>] [--hostname <string>] [--cors <origin>]
```
...
```bash
code-harmony serve --cors http://localhost:5173 --cors https://app.example.com
```
...
Set `CODE_HARMONY_SERVER_PASSWORD` to protect the server with HTTP basic auth. The username defaults to `code-harmony`, or set `CODE_HARMONY_SERVER_USERNAME` to override it. This applies to both `code-harmony serve` and `code-harmony web`.

```bash
CODE_HARMONY_SERVER_PASSWORD=your-password code-harmony serve
```
...
When you run `code-harmony` it starts a TUI and a server. Where the TUI is the
...
:::tip
Use the code-harmony server to interact with code-harmony programmatically.
:::

This architecture lets code-harmony support multiple clients and allows you to interact with code-harmony programmatically.

You can run `code-harmony serve` to start a standalone server. If you have the
code-harmony TUI running, `code-harmony serve` will start a new server.
...
The [`/tui`](#tui) endpoint can be used to drive the TUI through the server. For example, you can prefill or run a prompt. This setup is used by the CodeHarmony [IDE](/docs/ide) plugins.
...
The code-harmony server exposes the following APIs.
