---
title: Server
description: Interact with emberharmony server over HTTP.
---

import config from "../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

The `emberharmony serve` command runs a headless HTTP server that exposes an OpenAPI endpoint that an emberharmony client can use.
...
```bash
emberharmony serve [--port <number>] [--hostname <string>] [--cors <origin>]
```
...
```bash
emberharmony serve --cors http://localhost:5173 --cors https://app.example.com
```
...
Set `EMBERHARMONY_SERVER_PASSWORD` to protect the server with HTTP basic auth. The username defaults to `emberharmony`, or set `EMBERHARMONY_SERVER_USERNAME` to override it. This applies to both `emberharmony serve` and `emberharmony web`.

```bash
EMBERHARMONY_SERVER_PASSWORD=your-password emberharmony serve
```
...
When you run `emberharmony` it starts a TUI and a server. Where the TUI is the
...
:::tip
Use the emberharmony server to interact with emberharmony programmatically.
:::

This architecture lets emberharmony support multiple clients and allows you to interact with emberharmony programmatically.

You can run `emberharmony serve` to start a standalone server. If you have the
emberharmony TUI running, `emberharmony serve` will start a new server.
...
The [`/tui`](#tui) endpoint can be used to drive the TUI through the server. For example, you can prefill or run a prompt. This setup is used by the EmberHarmony [IDE](/docs/ide) plugins.
...
The emberharmony server exposes the following APIs.
