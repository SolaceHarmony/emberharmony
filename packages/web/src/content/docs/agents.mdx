...
There are two types of agents in EmberHarmony; primary agents and subagents.
...
EmberHarmony comes with two built-in primary agents, **Build** and **Plan**. We'll
look at these below.
...
EmberHarmony comes with two built-in subagents, **General** and **Explore**. We'll look at this below.

---

## Built-in

EmberHarmony comes with two built-in primary agents and two built-in subagents.
...
Configure agents in your `emberharmony.json` config file:
...
- Global: `~/.config/emberharmony/agents/`
- Per-project: `.emberharmony/agents/`

```markdown title="~/.config/emberharmony/agents/review.md"
---
...
If no temperature is specified, EmberHarmony uses model-specific defaults; typically 0 for most models, 0.55 for Qwen models.
...
This path is relative to where the config file is located. So this works for both the global EmberHarmony config and the project specific config.
...
The model ID in your EmberHarmony config uses the format `provider/model-id`. For example, if you're using [EmberHarmony Zen](/docs/zen), you would use `emberharmony/gpt-5.1-codex` for GPT 5.1 Codex.
...
```json title="emberharmony.json" {3-6,9-12}
{
  "$schema": "https://solace.ofharmony.ai/config.json",
  "tools": {
    "write": true,
...
```json title="emberharmony.json" {3-5,8-10}
{
  "$schema": "https://solace.ofharmony.ai/config.json",
  "permission": {
    "edit": "deny"
...
```json title="emberharmony.json" {8}
{
  "$schema": "https://solace.ofharmony.ai/config.json",
  "agent": {
    "build": {
...
```json title="emberharmony.json" {6,7}
{
  "agent": {
    "deep-thinker": {
...
:::tip
Run `emberharmony models` to see a list of the available models.
:::
...
:::tip
Do you have an agent you'd like to share? [Submit a PR](https://github.com/sydneyrenee/emberharmony).
:::
...
```markdown title="~/.config/emberharmony/agents/docs-writer.md"
---
...
```markdown title="~/.config/emberharmony/agents/security-auditor.md"
---
