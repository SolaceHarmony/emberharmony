...
There are two types of agents in CodeHarmony; primary agents and subagents.
...
CodeHarmony comes with two built-in primary agents, **Build** and **Plan**. We'll
look at these below.
...
CodeHarmony comes with two built-in subagents, **General** and **Explore**. We'll look at this below.

---

## Built-in

CodeHarmony comes with two built-in primary agents and two built-in subagents.
...
Configure agents in your `code-harmony.json` config file:
...
- Global: `~/.config/code-harmony/agents/`
- Per-project: `.code-harmony/agents/`

```markdown title="~/.config/code-harmony/agents/review.md"
---
...
If no temperature is specified, CodeHarmony uses model-specific defaults; typically 0 for most models, 0.55 for Qwen models.
...
This path is relative to where the config file is located. So this works for both the global CodeHarmony config and the project specific config.
...
The model ID in your CodeHarmony config uses the format `provider/model-id`. For example, if you're using [CodeHarmony Zen](/docs/zen), you would use `code-harmony/gpt-5.1-codex` for GPT 5.1 Codex.
...
```json title="code-harmony.json" {3-6,9-12}
{
  "$schema": "https://solace.ofharmony.ai/config.json",
  "tools": {
    "write": true,
...
```json title="code-harmony.json" {3-5,8-10}
{
  "$schema": "https://solace.ofharmony.ai/config.json",
  "permission": {
    "edit": "deny"
...
```json title="code-harmony.json" {8}
{
  "$schema": "https://solace.ofharmony.ai/config.json",
  "agent": {
    "build": {
...
```json title="code-harmony.json" {6,7}
{
  "agent": {
    "deep-thinker": {
...
:::tip
Run `code-harmony models` to see a list of the available models.
:::
...
:::tip
Do you have an agent you'd like to share? [Submit a PR](https://github.com/sydneyrenee/code-harmony).
:::
...
```markdown title="~/.config/code-harmony/agents/docs-writer.md"
---
...
```markdown title="~/.config/code-harmony/agents/security-auditor.md"
---
