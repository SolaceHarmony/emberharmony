...
CodeHarmony has a list of keybinds that you can customize through the CodeHarmony config.
...
CodeHarmony uses a `leader` key for most keybinds. This avoids conflicts in your terminal.
...
The CodeHarmony desktop app prompt input supports common Readline/Emacs-style shortcuts for editing text. These are built-in and currently not configurable via `code-harmony.json`.
