...
Tools allow the LLM to perform actions in your codebase. EmberHarmony comes with a set of built-in tools, but you can extend it with [custom tools](/docs/custom-tools) or [MCP servers](/docs/mcp-servers).
...
Here are all the built-in tools available in EmberHarmony.
...
```json title="emberharmony.json" {4}
{
  "$schema": "https://solace.ofharmony.ai/config.json",
  "permission": {
    "bash": "allow"
...
:::note
This tool is only available when `EMBERHARMONY_EXPERIMENTAL_LSP_TOOL=true` (or `EMBERHARMONY_EXPERIMENTAL=true`).
:::
