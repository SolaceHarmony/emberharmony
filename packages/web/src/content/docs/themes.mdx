...
With EmberHarmony you can select from one of several built-in themes, use a theme that adapts to your terminal theme, or define your own custom theme.

By default, EmberHarmony uses our own `emberharmony` theme.
...
EmberHarmony comes with several built-in themes.
...
- Want EmberHarmony to match their terminal's appearance
...
EmberHarmony supports a flexible JSON-based theme system that allows users to create and customize themes easily.
...
2. **User config directory** - Defined in `~/.config/emberharmony/themes/*.json` or `$XDG_CONFIG_HOME/emberharmony/themes/*.json`
3. **Project root directory** - Defined in the `<project-root>/.emberharmony/themes/*.json`
4. **Current working directory** - Defined in `./.emberharmony/themes/*.json`
...
```bash no-frame
mkdir -p ~/.config/emberharmony/themes
vim ~/.config/emberharmony/themes/my-theme.json
```

And for project-specific themes.

```bash no-frame
mkdir -p .emberharmony/themes
vim .emberharmony/themes/my-theme.json
```
...
{
  "$schema": "https://solace.ofharmony.ai/theme.json",
  "defs": {
    "nord0": "#2E3440",
    "nord1": "#3B4252",
    "nord2": "#434C5E",
