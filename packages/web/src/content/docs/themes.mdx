...
With CodeHarmony you can select from one of several built-in themes, use a theme that adapts to your terminal theme, or define your own custom theme.

By default, CodeHarmony uses our own `code-harmony` theme.
...
CodeHarmony comes with several built-in themes.
...
- Want CodeHarmony to match their terminal's appearance
...
CodeHarmony supports a flexible JSON-based theme system that allows users to create and customize themes easily.
...
2. **User config directory** - Defined in `~/.config/code-harmony/themes/*.json` or `$XDG_CONFIG_HOME/code-harmony/themes/*.json`
3. **Project root directory** - Defined in the `<project-root>/.code-harmony/themes/*.json`
4. **Current working directory** - Defined in `./.code-harmony/themes/*.json`
...
```bash no-frame
mkdir -p ~/.config/code-harmony/themes
vim ~/.config/code-harmony/themes/my-theme.json
```

And for project-specific themes.

```bash no-frame
mkdir -p .code-harmony/themes
vim .code-harmony/themes/my-theme.json
```
