---
title: LSP Servers
description: CodeHarmony integrates with your LSP servers.
---

CodeHarmony integrates with your Language Server Protocol (LSP) to help the LLM interact with your codebase. It uses diagnostics to provide feedback to the LLM.

---

## Built-in

CodeHarmony comes with several built-in LSP servers for popular languages:
...
:::note
You can disable automatic LSP server downloads by setting the `CODE_HARMONY_DISABLE_LSP_DOWNLOAD` environment variable to `true`.
:::
...
When code-harmony opens a file, it:
...
You can customize LSP servers through the `lsp` section in your code-harmony config.
