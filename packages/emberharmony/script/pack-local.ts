...
const main = async () => {
  ...
  const name = "name" in pkg && typeof pkg.name === "string" ? pkg.name : "emberharmony"
  ...
  const bin = platform === "windows" ? `${cli}.exe` : cli
  const ext = platform === "windows" ? ".exe" : ""

  if (!nobuild) {
    await $`EMBERHARMONY_CHANNEL=latest EMBERHARMONY_VERSION=${ver} bun run script/build.ts --single`
  }
