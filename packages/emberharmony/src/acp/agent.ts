...
import type { Event, EmberHarmonyClient, SessionMessageResponse } from "@thesolaceproject/emberharmony-sdk/v2"
...
export namespace ACP {
  ...
  export async function init({ sdk: _sdk }: { sdk: EmberHarmonyClient }) {
    return {
  ...
  export class Agent implements ACPAgent {
    ...
    private sdk: EmberHarmonyClient
    ...
    async initialize(params: InitializeRequest): Promise<InitializeResponse> {
      ...
      const authMethod: AuthMethod = {
        description: "Run `emberharmony auth login` in the terminal",
        name: "Login with EmberHarmony",
        id: "emberharmony-login",
      }
      ...
      if (params.clientCapabilities?._meta?.["terminal-auth"] === true) {
        authMethod._meta = {
          "terminal-auth": {
            command: "emberharmony",
            args: ["auth", "login"],
            label: "EmberHarmony Login",
          },
        }
      }
    ...
    private async processMessage(message: SessionMessageResponse) {
      ...
      for (const part of message.parts) {
        if (part.type === "tool") {
          ...
          // EmberHarmony stores files internally as { type: "file", url, filename, mime }.
  ...
  async function defaultModel(config: ACPConfig, cwd?: string) {
    ...
    const harmony = providers.find((p) => p.id === "emberharmony")
    if (harmony) {
      if (harmony.models["big-pickle"]) {
        return { providerID: "emberharmony", modelID: "big-pickle" }
      }
      const [best] = Provider.sort(Object.values(harmony.models))
      if (best) {
        return {
