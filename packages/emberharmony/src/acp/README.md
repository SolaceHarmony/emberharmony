...
This directory contains a clean, protocol-compliant implementation of the [Agent Client Protocol](https://agentclientprotocol.com/) for emberharmony.
...
- **`session.ts`** - Session state management
  ...
  - Maps ACP sessions to internal emberharmony sessions
...
emberharmony acp --cwd /path/to/project
```
...
- **Terminal Integration**: Full terminal support via emberharmony's bash tool
...
echo '{"jsonrpc":"2.0","id":1,"method":"initialize","params":{"protocolVersion":1}}' | emberharmony acp
```
...
### Mapping to EmberHarmony

ACP sessions map cleanly to emberharmony's internal session model:
