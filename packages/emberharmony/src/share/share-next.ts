...
import type * as SDK from "@thesolaceproject/emberharmony-sdk/v2"

export namespace ShareNext {
  const log = Log.create({ service: "share-next" })

  async function url() {
    return Config.get().then((x) => x.enterprise?.url ?? "https://solace.ofharmony.ai")
  }

  const disabled =
    process.env["EMBERHARMONY_DISABLE_SHARE"] === "true" || process.env["EMBERHARMONY_DISABLE_SHARE"] === "1"
