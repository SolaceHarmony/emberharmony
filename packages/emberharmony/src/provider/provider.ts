...
export namespace Provider {
  ...
  const CUSTOM_LOADERS: Record<string, CustomLoader> = {
    ...
    "emberharmony": async (input) => {
      const hasKey = await (async () => {
        ...
        if (config.provider?.["emberharmony"]?.options?.apiKey) return true
        return false
      })()
    ...
    "amazon-bedrock": async () => {
      ...
      return {
        ...
        async getModel(sdk: any, modelID: string, options?: Record<string, any>) {
          ...
          // 1. options.region from emberharmony.json provider config
    ...
    cerebras: async () => {
      return {
        ...
        options: {
          headers: {
            "X-Cerebras-3rd-Party-Integration": "emberharmony",
          },
        },
      }
    },
  ...
  const state = Instance.state(async () => {
    ...
    for (const [providerID, fn] of Object.entries(CUSTOM_LOADERS)) {
      ...
      if (!data) {
        if (providerID === "emberharmony") continue
    ...
    for (const [providerID, provider] of Object.entries(providers)) {
      ...
      for (const [modelID, model] of Object.entries(provider.models)) {
        ...
        if (model.status === "alpha" && !Flag.EMBERHARMONY_ENABLE_EXPERIMENTAL_MODELS) delete provider.models[modelID]
  ...
  export async function getSmallModel(providerID: string) {
    ...
    if (provider) {
      ...
      if (providerID.startsWith("emberharmony")) {
        priority = ["gpt-5-nano"]
      }
    ...
    // Check if emberharmony provider is available before using it
    const harmonyProvider = await state().then((state) => state.providers["emberharmony"])
    if (harmonyProvider && harmonyProvider.models["gpt-5-nano"]) {
      return getModel("emberharmony", "gpt-5-nano")
    }
