...
import harmony from "./theme/emberharmony.json" with { type: "json" }
...
export function selectedForeground(theme: Theme, bg?: RGBA): RGBA {
  ...
  const target = bg ?? (theme.background.a === 0 ? theme.primary : undefined)
  if (target) {
    const { r, g, b } = target
...
export const { use: useTheme, provider: ThemeProvider } = createSimpleContext({
  ...
  init: (props: { mode: "dark" | "light" }) => {
    ...
    const [store, setStore] = createStore({
      ...
      active: (sync.data.config.theme ?? kv.get("theme", "emberharmony")) as string,
      ready: false,
    })
    ...
    const values = createMemo(() => {
      return resolveTheme(store.themes[store.active] ?? store.themes["emberharmony"], store.mode)
    })
