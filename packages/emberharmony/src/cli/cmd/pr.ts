...
export const PrCommand = cmd({
  command: "pr <number>",
  describe: "fetch and checkout a GitHub PR branch, then run emberharmony",
  builder: (yargs) =>
      ...
      async fn() {
        ...
        if (prInfoResult.exitCode === 0) {
          ...
          if (prInfoText.trim()) {
            ...
            // Check for EmberHarmony session link in PR body
            if (prInfo && prInfo.body) {
              const sessionMatch = prInfo.body.match(/https:\/\/solace\.ofharmony\.ai\/s\/([a-zA-Z0-9_-]+)/)
              if (sessionMatch) {
                const sessionUrl = sessionMatch[0]
                UI.println(`Found EmberHarmony session: ${sessionUrl}`)
                UI.println(`Importing session...`)

                const importResult = await $`emberharmony import ${sessionUrl}`.nothrow()
        ...
        UI.println()
        UI.println("Starting emberharmony...")
        UI.println()

        // Launch EmberHarmony TUI with session ID if available
        const { spawn } = await import("child_process")
        const argv = sessionId ? ["-s", sessionId] : []
        const proc = spawn("emberharmony", argv, {
          stdio: "inherit",
          cwd: process.cwd(),
        })
        ...
        await new Promise<void>((resolve, reject) => {
          proc.on("exit", (code) => {
            if (code === 0) resolve()
            else reject(new Error(`emberharmony exited with code ${code}`))
          })
