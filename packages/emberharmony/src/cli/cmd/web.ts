...
export const WebCommand = cmd({
  ...
  describe: "start emberharmony server and open web interface",
  handler: async (args) => {
    if (!Flag.EMBERHARMONY_SERVER_PASSWORD) {
      UI.println(UI.Style.TEXT_WARNING_BOLD + "!  " + "EMBERHARMONY_SERVER_PASSWORD is not set; server is unsecured.")
    }
    ...
    if (opts.hostname === "0.0.0.0") {
      ...
      if (opts.mdns) {
        UI.println(UI.Style.TEXT_INFO_BOLD + "  mDNS:              ", UI.Style.TEXT_NORMAL, `emberharmony.local:${server.port}`)
      }
    }

    const mode = process.env.EMBERHARMONY_WEB_UI ?? "auto"
    const noopen = process.env.EMBERHARMONY_WEB_NO_OPEN === "1"

    // Search for the app package in several likely locations
    const candidates = [
      path.resolve(process.cwd(), "..", "app"),           // CWD is packages/emberharmony
      path.resolve(process.cwd(), "packages", "app"),     // CWD is repo root
      path.resolve(process.cwd(), "app"),                 // CWD is packages/
    ]
    ...
    const useLocal = mode === "local" || (mode === "auto" && !!appdir && !!bunbin)
    if (!useLocal) {
    ...
    const child = Bun.spawn({
      ...
      env: {
        ...
        VITE_EMBERHARMONY_SERVER_PORT: String(server.port),
      },
