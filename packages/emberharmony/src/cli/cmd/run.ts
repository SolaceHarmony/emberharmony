...
import { createEmberHarmonyClient, type EmberHarmonyClient } from "@thesolaceproject/emberharmony-sdk/v2"
...
export const RunCommand = cmd({
  command: "run [message..]",
  describe: "run emberharmony with a message",
  builder: (yargs: Argv) => {
    return yargs
      .positional("message", {
      ...
      .option("attach", {
        type: "string",
        describe: "attach to a running emberharmony server (e.g., http://localhost:4096)",
      })
      .option("port", {
        type: "number",
  ...
  handler: async (args) => {
    ...
    const execute = async (sdk: EmberHarmonyClient, sessionID: string) => {
    ...
    if (args.attach) {
      const sdk = createEmberHarmonyClient({ baseUrl: args.attach })
      ...
      if (cfgResult.data && (cfgResult.data.share === "auto" || Flag.EMBERHARMONY_AUTO_SHARE || args.share)) {
    ...
    await bootstrap(process.cwd(), async () => {
      ...
      const sdk = createEmberHarmonyClient({ baseUrl: "http://emberharmony.internal", fetch: fetchFn })
