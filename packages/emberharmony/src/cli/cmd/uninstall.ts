...
export const UninstallCommand = {
  command: "uninstall",
  describe: "uninstall emberharmony and remove all related files",
  builder: (yargs: Argv) =>
    yargs
      .option("keep-config", {
        alias: "c",
  ...
  handler: async (args: UninstallArgs) => {
    ...
    UI.println(UI.logo("  "))
    UI.empty()
    prompts.intro("Uninstall EmberHarmony")
...
async function showRemovalSummary(targets: RemovalTargets, method: Installation.Method) {
  ...
  if (method !== "curl" && method !== "unknown") {
    const cmds: Record<string, string> = {
      npm: "npm uninstall -g emberharmony",
...
async function executeUninstall(method: Installation.Method, targets: RemovalTargets) {
  ...
  if (method === "curl" && targets.binary) {
    ...
    if (binDir.includes(".emberharmony")) {
  ...
  UI.empty()
  prompts.log.success("Thank you for using EmberHarmony!")
}
...
async function getShellConfigFile(): Promise<string | null> {
  ...
  for (const file of candidates) {
    ...
    const content = await Bun.file(file)
      .text()
      .catch(() => "")
    if (content.includes("# emberharmony") || content.includes(".emberharmony/bin")) {
      return file
    }
  }
...
async function cleanShellConfig(file: string) {
  ...
  let skip = false
  const markers = new Set(["# emberharmony"])
  const pathParts = [".emberharmony/bin"]
  ...
  for (const line of lines) {
    ...
    if (
      ...
      (trimmed.startsWith("fish_add_path") && trimmed.includes(".emberharmony"))
    ) {
      continue
    }
