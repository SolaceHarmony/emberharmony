...
export const ImportCommand = cmd({
  ...
  builder: (yargs: Argv) => {
    return yargs.positional("file", {
      describe: "path to JSON file or solace.ofharmony.ai share URL",
      type: "string",
      demandOption: true,
    })
  },
  handler: async (args) => {
    await bootstrap(process.cwd(), async () => {
      ...
      if (isUrl) {
        const urlMatch = args.file.match(/https?:\/\/solace\.ofharmony\.ai\/(?:s|share)\/([a-zA-Z0-9_-]+)/)
        if (!urlMatch) {
          process.stdout.write(`Invalid URL format. Expected: https://solace.ofharmony.ai/s/<id>`)
          process.stdout.write(EOL)
          return
        }

        const slug = urlMatch[1]
        const response = await fetch(`https://solace.ofharmony.ai/api/share/${slug}`)
