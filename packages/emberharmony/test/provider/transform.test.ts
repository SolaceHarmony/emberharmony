...
describe("ProviderTransform.message - strip openai metadata when store=false", () => {
  ...
  test("preserves metadata using providerID key when store is false", () => {
    const harmonyModel = {
      ...openaiModel,
      providerID: "emberharmony",
      api: {
        id: "emberharmony-test",
        url: "https://api.solace.ofharmony.ai",
        npm: "@ai-sdk/openai-compatible",
      },
    }
    const msgs = [
    ...
    expect(result[0].content[0].providerOptions?.["emberharmony"]?.itemId).toBe("msg_123")
    expect(result[0].content[0].providerOptions?.["emberharmony"]?.otherOption).toBe("value")
  })
  ...
  test("preserves itemId across all providerOptions keys", () => {
    ...
    const msgs = [
      {
        ...
        providerOptions: {
          ...
          "emberharmony": { itemId: "msg_harmony" },
        ...
        content: [
          {
            ...
            providerOptions: {
              ...
              "emberharmony": { itemId: "msg_harmony_part" },
    ...
    expect(result[0].providerOptions?.["emberharmony"]?.itemId).toBe("msg_harmony")
    expect(result[0].providerOptions?.extra?.itemId).toBe("msg_extra")
    expect(result[0].content[0].providerOptions?.openai?.itemId).toBe("msg_openai_part")
    expect(result[0].content[0].providerOptions?.["emberharmony"]?.itemId).toBe("msg_harmony_part")
