...
const managedConfigDir = process.env.EMBERHARMONY_TEST_MANAGED_CONFIG_DIR!

afterEach(async () => {
  await fs.rm(managedConfigDir, { force: true, recursive: true }).catch(() => {})
})

async function writeManagedSettings(settings: object, filename = "emberharmony.json") {
  await fs.mkdir(managedConfigDir, { recursive: true })
  await Bun.write(path.join(managedConfigDir, filename), JSON.stringify(settings))
}

async function writeConfig(dir: string, config: object, name = "emberharmony.json") {
...
test("loads JSONC config file", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await Bun.write(
        path.join(dir, "emberharmony.jsonc"),
        `{
        // This is a comment
        "$schema": "https://solace.ofharmony.ai/config.json",
        "model": "test/model",
  ...
  try {
    ...
    await Instance.provide({
      ...
      fn: async () => {
        ...
        const content = await Bun.file(path.join(tmp.path, "emberharmony.json")).text()
...
test("throws error for invalid JSON", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await Bun.write(path.join(dir, "emberharmony.json"), "{ invalid json }")
    },
  })
  await Instance.provide({
    directory: tmp.path,
...
test("loads config from .emberharmony directory", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const harmony = path.join(dir, ".emberharmony")
...
test("loads agents from .emberharmony/agents (plural)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const harmony = path.join(dir, ".emberharmony")
      await fs.mkdir(harmony, { recursive: true })
...
test("loads commands from .emberharmony/command (singular)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const harmony = path.join(dir, ".emberharmony")
      await fs.mkdir(harmony, { recursive: true })
...
test("loads commands from .emberharmony/commands (plural)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const harmony = path.join(dir, ".emberharmony")
      await fs.mkdir(harmony, { recursive: true })
...
test("resolves scoped npm plugins in config", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      await Bun.write(
        path.join(dir, "emberharmony.json"),
        JSON.stringify({ $schema: "https://solace.ofharmony.ai/config.json", plugin: ["@scope/plugin"] }, null, 2),
      )
    },
  })
  ...
  await Instance.provide({
    ...
    fn: async () => {
      ...
      const baseUrl = pathToFileURL(path.join(tmp.path, "emberharmony.json")).href
...
test("merges plugin arrays from global and local configs", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      // Create a nested project structure with local .emberharmony config
      const projectDir = path.join(dir, "project")
      const harmony = path.join(projectDir, ".emberharmony")
      await fs.mkdir(harmony, { recursive: true })
      ...
      // Local .emberharmony config with different plugins
      await Bun.write(
        path.join(harmony, "emberharmony.json"),
        JSON.stringify({
          $schema: "https://solace.ofharmony.ai/config.json",
...
test("deduplicates duplicate plugins from global and local configs", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      // Local .emberharmony config with some overlapping plugins
      await Bun.write(
        path.join(harmony, "emberharmony.json"),
        JSON.stringify({
          $schema: "https://solace.ofharmony.ai/config.json",
...
// Note: preload.ts sets EMBERHARMONY_TEST_MANAGED_CONFIG_DIR which Global.Path.managedConfig uses
...
test("local .emberharmony config can override MCP from project config", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      // Local .emberharmony directory config enables it
      const harmony = path.join(dir, ".emberharmony")
      await fs.mkdir(harmony, { recursive: true })
      await Bun.write(
        path.join(harmony, "emberharmony.json"),
...
test("project config overrides remote well-known config", async () => {
  ...
  const mockFetch = mock((url: string | URL | Request) => {
    ...
    if (urlStr.includes(".well-known/emberharmony")) {
      fetchedUrl = urlStr
  ...
  try {
    ...
    await Instance.provide({
      ...
      fn: async () => {
        ...
        expect(fetchedUrl).toBe("https://example.com/.well-known/emberharmony")
...
describe("getPluginName", () => {
  ...
  test("extracts name from npm package with version", () => {
    expect(Config.getPluginName("oh-my-emberharmony@2.4.3")).toBe("oh-my-emberharmony")
  ...
  test("extracts name from scoped npm package", () => {
    ...
    expect(Config.getPluginName("@thesolaceproject/emberharmony-plugin@2.0.0")).toBe(
      "@thesolaceproject/emberharmony-plugin",
    )
  })
...
describe("deduplicatePlugins", () => {
  ...
  test("prefers local file over npm package with same name", () => {
    const plugins = ["oh-my-emberharmony@2.4.3", "file:///project/.emberharmony/plugin/oh-my-emberharmony.js"]

    const result = Config.deduplicatePlugins(plugins)

    expect(result.length).toBe(1)
    expect(result[0]).toBe("file:///project/.emberharmony/plugin/oh-my-emberharmony.js")
  })
  ...
  test("local plugin directory overrides global emberharmony.json plugin", async () => {
    await using tmp = await tmpdir({
      init: async (dir) => {
        const projectDir = path.join(dir, "project")
        const harmony = path.join(projectDir, ".emberharmony")
...
describe("EMBERHARMONY_DISABLE_PROJECT_CONFIG", () => {
  test("skips project config files when flag is set", async () => {
    const originalEnv = process.env["EMBERHARMONY_DISABLE_PROJECT_CONFIG"]
    process.env["EMBERHARMONY_DISABLE_PROJECT_CONFIG"] = "true"
    ...
    try {
      ...
      if (originalEnv === undefined) {
        delete process.env["EMBERHARMONY_DISABLE_PROJECT_CONFIG"]
      } else {
        process.env["EMBERHARMONY_DISABLE_PROJECT_CONFIG"] = originalEnv
      }
    }
  })

  test("skips project .emberharmony/ directories when flag is set", async () => {
    const originalEnv = process.env["EMBERHARMONY_DISABLE_PROJECT_CONFIG"]
    process.env["EMBERHARMONY_DISABLE_PROJECT_CONFIG"] = "true"
    ...
    try {
      await using tmp = await tmpdir({
        init: async (dir) => {
          // Create a .emberharmony directory with a command
          const harmony = path.join(dir, ".emberharmony", "command")
          await fs.mkdir(harmony, { recursive: true })
      ...
      await Instance.provide({
        ...
        fn: async () => {
          ...
          // Project .emberharmony should NOT be in directories list
  ...
  test("skips relative instructions with warning when flag is set but no config dir", async () => {
    const originalDisable = process.env["EMBERHARMONY_DISABLE_PROJECT_CONFIG"]
    const originalConfigDir = process.env["EMBERHARMONY_CONFIG_DIR"]

    try {
      // Ensure no config dir is set
      delete process.env["EMBERHARMONY_CONFIG_DIR"]
      process.env["EMBERHARMONY_DISABLE_PROJECT_CONFIG"] = "true"
      ...
      if (originalDisable === undefined) {
        ...
        process.env["EMBERHARMONY_DISABLE_PROJECT_CONFIG"] = originalDisable
      }
      if (originalConfigDir === undefined) {
        delete process.env["EMBERHARMONY_CONFIG_DIR"]
      } else {
        process.env["EMBERHARMONY_CONFIG_DIR"] = originalConfigDir
      }
    }
  })

  test("EMBERHARMONY_CONFIG_DIR still works when flag is set", async () => {
    const originalDisable = process.env["EMBERHARMONY_DISABLE_PROJECT_CONFIG"]
    const originalConfigDir = process.env["EMBERHARMONY_CONFIG_DIR"]
    ...
    try {
      ...
      process.env["EMBERHARMONY_DISABLE_PROJECT_CONFIG"] = "true"
      process.env["EMBERHARMONY_CONFIG_DIR"] = configDirTmp.path

      await Instance.provide({
        directory: projectTmp.path,
        fn: async () => {
          const config = await Config.get()
          // Should load from EMBERHARMONY_CONFIG_DIR, not project
