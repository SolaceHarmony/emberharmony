...
export namespace Provider {
  ...
  const CUSTOM_LOADERS: Record<string, CustomLoader> = {
    ...
    "code-harmony": async (input) => {
      const hasKey = await (async () => {
        const env = Env.all()
        if (input.env.some((item) => env[item])) return true
        if (await Auth.get(input.id)) return true
        const config = await Config.get()
        if (config.provider?.["code-harmony"]?.options?.apiKey) return true
        return false
      })()
    ...
    "amazon-bedrock": async () => {
      ...
      return {
        ...
        async getModel(sdk: any, modelID: string, options?: Record<string, any>) {
          ...
          // 1. options.region from code-harmony.json provider config
    ...
    openrouter: async () => {
      return {
        ...
        options: {
          headers: {
            "HTTP-Referer": "https://solace.ofharmony.ai/",
            "X-Title": "code-harmony",
          },
        },
      }
    },
    ...
    cerebras: async () => {
      return {
        ...
        options: {
          headers: {
            "X-Cerebras-3rd-Party-Integration": "code-harmony",
          },
        },
      }
    },
  ...
  const state = Instance.state(async () => {
    ...
    for (const [providerID, fn] of Object.entries(CUSTOM_LOADERS)) {
      ...
      if (!data) {
        if (providerID === "code-harmony") continue
    ...
    for (const [providerID, provider] of Object.entries(providers)) {
      ...
      for (const [modelID, model] of Object.entries(provider.models)) {
        ...
        if (model.status === "alpha" && !Flag.CODE_HARMONY_ENABLE_EXPERIMENTAL_MODELS) delete provider.models[modelID]
  ...
  export async function getSmallModel(providerID: string) {
    ...
    if (provider) {
      ...
      if (providerID.startsWith("code-harmony")) {
        priority = ["gpt-5-nano"]
      }
    ...
    // Check if code-harmony provider is available before using it
    const harmonyProvider = await state().then((state) => state.providers["code-harmony"])
    if (harmonyProvider && harmonyProvider.models["gpt-5-nano"]) {
      return getModel("code-harmony", "gpt-5-nano")
    }
