...
import { createCodeHarmonyClient, type CodeHarmonyClient } from "@thesolaceproject/code-harmony-sdk/v2"
...
export const RunCommand = cmd({
  command: "run [message..]",
  describe: "run code-harmony with a message",
  builder: (yargs: Argv) => {
    return yargs
      .positional("message", {
      ...
      .option("attach", {
        type: "string",
        describe: "attach to a running code-harmony server (e.g., http://localhost:4096)",
      })
      .option("port", {
        type: "number",
  ...
  handler: async (args) => {
    ...
    const execute = async (sdk: CodeHarmonyClient, sessionID: string) => {
    ...
    if (args.attach) {
      const sdk = createCodeHarmonyClient({ baseUrl: args.attach })
      ...
      if (cfgResult.data && (cfgResult.data.share === "auto" || Flag.CODE_HARMONY_AUTO_SHARE || args.share)) {
    ...
    await bootstrap(process.cwd(), async () => {
      ...
      const sdk = createCodeHarmonyClient({ baseUrl: "http://code-harmony.internal", fetch: fetchFn })
