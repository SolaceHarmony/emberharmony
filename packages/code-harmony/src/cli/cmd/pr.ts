...
export const PrCommand = cmd({
  command: "pr <number>",
  describe: "fetch and checkout a GitHub PR branch, then run code-harmony",
  builder: (yargs) =>
      ...
      async fn() {
        ...
        if (prInfoResult.exitCode === 0) {
          ...
          if (prInfoText.trim()) {
            ...
            // Check for CodeHarmony session link in PR body
            if (prInfo && prInfo.body) {
              const sessionMatch = prInfo.body.match(/https:\/\/solace\.ofharmony\.ai\/s\/([a-zA-Z0-9_-]+)/)
              if (sessionMatch) {
                const sessionUrl = sessionMatch[0]
                UI.println(`Found CodeHarmony session: ${sessionUrl}`)
                UI.println(`Importing session...`)

                const importResult = await $`code-harmony import ${sessionUrl}`.nothrow()
        ...
        UI.println()
        UI.println("Starting code-harmony...")
        UI.println()
        ...
        await new Promise<void>((resolve, reject) => {
          proc.on("exit", (code) => {
            if (code === 0) resolve()
            else reject(new Error(`code-harmony exited with code ${code}`))
          })
