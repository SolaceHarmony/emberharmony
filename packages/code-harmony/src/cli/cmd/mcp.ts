    ...
    await Instance.provide({
      ...
      async fn() {
        ...
        if (servers.length === 0) {
          ...
          prompts.outro("Add servers with: code-harmony mcp add")
          return
        }
    ...
    await Instance.provide({
      ...
      async fn() {
        ...
        if (oauthServers.length === 0) {
          ...
          prompts.log.info("Remote MCP servers support OAuth by default. Add a remote server in code-harmony.json:")
          prompts.log.info(`
  "mcp": {
    "my-server": {
      "type": "remote",
...
async function resolveConfigPath(baseDir: string, global = false) {
  // Check for existing config files (prefer .jsonc over .json, check .code-harmony/ subdirectory too)
  const candidates = [path.join(baseDir, "code-harmony.json"), path.join(baseDir, "code-harmony.jsonc")]

  if (!global) {
    candidates.push(path.join(baseDir, ".code-harmony", "code-harmony.json"), path.join(baseDir, ".code-harmony", "code-harmony.jsonc"))
  }
  ...
  // Default to code-harmony.json if none exist
  return candidates[0]
}
      ...
      async fn() {
        ...
        if (type === "local") {
          const command = await prompts.text({
            ...
            placeholder: "e.g., code-harmony x @modelcontextprotocol/server-filesystem",
            validate: (x) => (x && x.length > 0 ? undefined : "Required"),
          })
          ...
          const response = await fetch(serverConfig.url, {
            ...
            body: JSON.stringify({
              ...
              params: {
                ...
                capabilities: {},
                clientInfo: { name: "code-harmony-debug", version: Installation.VERSION },
              },
              id: 1,
            }),
          })
