...
export const UninstallCommand = {
  ...
  handler: async (args: UninstallArgs) => {
    ...
    UI.println(UI.logo("  "))
    UI.empty()
    prompts.intro("Uninstall CodeHarmony")
...
async function executeUninstall(method: Installation.Method, targets: RemovalTargets) {
  ...
  if (method === "curl" && targets.binary) {
    ...
    if (binDir.includes(".code-harmony")) {
  ...
  UI.empty()
  prompts.log.success("Thank you for using CodeHarmony!")
}
...
async function cleanShellConfig(file: string) {
  ...
  let skip = false
  const markers = new Set(["# code-harmony"])
  const pathParts = [".code-harmony/bin"]
  ...
  for (const line of lines) {
    ...
    if (
      ...
      (trimmed.startsWith("fish_add_path") && trimmed.includes(".code-harmony"))
    ) {
      continue
    }
