...
import type { Event, CodeHarmonyClient, SessionMessageResponse } from "@thesolaceproject/code-harmony-sdk/v2"
...
export namespace ACP {
  ...
  export async function init({ sdk: _sdk }: { sdk: CodeHarmonyClient }) {
    return {
  ...
  export class Agent implements ACPAgent {
    ...
    async initialize(params: InitializeRequest): Promise<InitializeResponse> {
      ...
      const authMethod: AuthMethod = {
        description: "Run `code-harmony auth login` in the terminal",
        name: "Login with CodeHarmony",
        id: "code-harmony-login",
      }
      ...
      if (params.clientCapabilities?._meta?.["terminal-auth"] === true) {
        authMethod._meta = {
          "terminal-auth": {
            command: "code-harmony",
            args: ["auth", "login"],
            label: "CodeHarmony Login",
          },
        }
      }
    ...
    private async processMessage(message: SessionMessageResponse) {
      ...
      for (const part of message.parts) {
        if (part.type === "tool") {
          ...
          // CodeHarmony stores files internally as { type: "file", url, filename, mime }.
  ...
  async function defaultModel(config: ACPConfig, cwd?: string) {
    ...
    const harmony = providers.find((p) => p.id === "code-harmony")
    if (harmony) {
      if (harmony.models["big-pickle"]) {
        return { providerID: "code-harmony", modelID: "big-pickle" }
      }
