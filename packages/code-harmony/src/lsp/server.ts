...
export namespace LSPServer {
    ...
    async spawn(root) {
      ...
      if (!binary) {
        ...
        if (!(await Bun.file(js).exists())) {
          if (Flag.CODE_HARMONY_DISABLE_LSP_DOWNLOAD) return
  ...
  export const Ty: Info = {
    ...
    async spawn(root) {
      if (!Flag.CODE_HARMONY_EXPERIMENTAL_LSP_TY) {
        return undefined
      }
  ...
  export const JDTLS: Info = {
    ...
    async spawn(root) {
      ...
      const dataDir = await fs.mkdtemp(path.join(os.tmpdir(), "code-harmony-jdtls-data"))
      return {
        process: spawn(
          java,
          [
