...
const managedConfigDir = process.env.CODE_HARMONY_TEST_MANAGED_CONFIG_DIR!

afterEach(async () => {
  await fs.rm(managedConfigDir, { force: true, recursive: true }).catch(() => {})
})

async function writeManagedSettings(settings: object, filename = "code-harmony.json") {
  await fs.mkdir(managedConfigDir, { recursive: true })
  await Bun.write(path.join(managedConfigDir, filename), JSON.stringify(settings))
}

async function writeConfig(dir: string, config: object, name = "code-harmony.json") {
...
test("loads JSONC config file", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await Bun.write(
        path.join(dir, "code-harmony.jsonc"),
        `{
        // This is a comment
        "$schema": "https://solace.ofharmony.ai/config.json",
        "model": "test/model",
  ...
  try {
    ...
    await Instance.provide({
      ...
      fn: async () => {
        ...
        const content = await Bun.file(path.join(tmp.path, "code-harmony.json")).text()
...
test("throws error for invalid JSON", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await Bun.write(path.join(dir, "code-harmony.json"), "{ invalid json }")
    },
  })
  await Instance.provide({
    directory: tmp.path,
...
test("resolves scoped npm plugins in config", async () => {
  ...
  await Instance.provide({
    ...
    fn: async () => {
      ...
      const baseUrl = pathToFileURL(path.join(tmp.path, "code-harmony.json")).href
...
// Note: preload.ts sets CODE_HARMONY_TEST_MANAGED_CONFIG_DIR which Global.Path.managedConfig uses
...
test("local .code-harmony config can override MCP from project config", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
...
test("project config overrides remote well-known config", async () => {
  ...
  const mockFetch = mock((url: string | URL | Request) => {
    ...
    if (urlStr.includes(".well-known/code-harmony")) {
      fetchedUrl = urlStr
  ...
  try {
    ...
    await Instance.provide({
      ...
      fn: async () => {
        ...
        expect(fetchedUrl).toBe("https://example.com/.well-known/code-harmony")
...
describe("getPluginName", () => {
  ...
  test("extracts name from npm package with version", () => {
    expect(Config.getPluginName("oh-my-code-harmony@2.4.3")).toBe("oh-my-code-harmony")
...
describe("deduplicatePlugins", () => {
  ...
  test("prefers local file over npm package with same name", () => {
    const plugins = ["oh-my-code-harmony@2.4.3", "file:///project/.code-harmony/plugin/oh-my-code-harmony.js"]

    const result = Config.deduplicatePlugins(plugins)

    expect(result.length).toBe(1)
    expect(result[0]).toBe("file:///project/.code-harmony/plugin/oh-my-code-harmony.js")
  })
  ...
  test("local plugin directory overrides global code-harmony.json plugin", async () => {
    await using tmp = await tmpdir({
      init: async (dir) => {
        const projectDir = path.join(dir, "project")
        const harmony = path.join(projectDir, ".code-harmony")
...
describe("CODE_HARMONY_DISABLE_PROJECT_CONFIG", () => {
  test("skips project config files when flag is set", async () => {
    const originalEnv = process.env["CODE_HARMONY_DISABLE_PROJECT_CONFIG"]
    process.env["CODE_HARMONY_DISABLE_PROJECT_CONFIG"] = "true"
    ...
    try {
      ...
      if (originalEnv === undefined) {
        delete process.env["CODE_HARMONY_DISABLE_PROJECT_CONFIG"]
      } else {
        process.env["CODE_HARMONY_DISABLE_PROJECT_CONFIG"] = originalEnv
      }
    }
  })

  test("skips project .code-harmony/ directories when flag is set", async () => {
    const originalEnv = process.env["CODE_HARMONY_DISABLE_PROJECT_CONFIG"]
    process.env["CODE_HARMONY_DISABLE_PROJECT_CONFIG"] = "true"
  ...
  test("skips relative instructions with warning when flag is set but no config dir", async () => {
    const originalDisable = process.env["CODE_HARMONY_DISABLE_PROJECT_CONFIG"]
    const originalConfigDir = process.env["CODE_HARMONY_CONFIG_DIR"]

    try {
      // Ensure no config dir is set
      delete process.env["CODE_HARMONY_CONFIG_DIR"]
      process.env["CODE_HARMONY_DISABLE_PROJECT_CONFIG"] = "true"
      ...
      if (originalDisable === undefined) {
        ...
        process.env["CODE_HARMONY_DISABLE_PROJECT_CONFIG"] = originalDisable
      }
      if (originalConfigDir === undefined) {
        delete process.env["CODE_HARMONY_CONFIG_DIR"]
      } else {
        process.env["CODE_HARMONY_CONFIG_DIR"] = originalConfigDir
      }
    }
  })

  test("CODE_HARMONY_CONFIG_DIR still works when flag is set", async () => {
    const originalDisable = process.env["CODE_HARMONY_DISABLE_PROJECT_CONFIG"]
    const originalConfigDir = process.env["CODE_HARMONY_CONFIG_DIR"]
    ...
    try {
      ...
      process.env["CODE_HARMONY_DISABLE_PROJECT_CONFIG"] = "true"
      process.env["CODE_HARMONY_CONFIG_DIR"] = configDirTmp.path

      await Instance.provide({
        directory: projectTmp.path,
        fn: async () => {
          const config = await Config.get()
          // Should load from CODE_HARMONY_CONFIG_DIR, not project
