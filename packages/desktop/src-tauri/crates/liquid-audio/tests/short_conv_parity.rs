...
fn run_bf16(h: usize, l: usize, k: usize, seed: u64) -> f32 {
    ...
    let bxb = Tensor::from_vec(bx, (b, h, l), &dev).unwrap().to_dtype(DType::BF16).unwrap();
    let wb = Tensor::from_vec(w, (h, k), &dev).unwrap().to_dtype(DType::BF16).unwrap();
    // reference: same bf16 inputs, conv in f32, then a single round to bf16.
    let reference = candle_flashfftconv::depthwise_conv1d(&bxb.to_dtype(DType::F32).unwrap(), &wb.to_dtype(DType::F32).unwrap(), None, k - 1)
        .unwrap().narrow(2, 0, l).unwrap().to_dtype(DType::BF16).unwrap().to_dtype(DType::F32).unwrap();
    ...
    let bf16 = candle_flashfftconv::depthwise_conv1d(&bxb, &wb, None, k - 1)
        .unwrap().narrow(2, 0, l).unwrap().to_dtype(DType::F32).unwrap();
...
fn depthwise_conv1d_bf16_is_f32_accum_bf16_store() {
    for (h, l, k) in [(2048usize, 64usize, 3usize), (512, 7, 3), (256, 33, 4)] {
        ...
        assert!(d < 1e-6, "bf16 depthwise != f32-accumulate/bf16-store (H={h} L={l} K={k}): max diff {d}");
