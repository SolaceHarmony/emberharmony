...
fn mel_exact_pad_parity() -> anyhow::Result<()> {
    ...
    assert_eq!(
        mel.dims(),
        mel_ref.dims(),
        "exact_pad mel shape mismatch (center=False framing)"
    );
...
fn rel_pos_attention_sdpa_parity() -> anyhow::Result<()> {
    ...
    let g = candle_core::safetensors::load(
        manifest.join("parity/golden/mha_sdpa_refs.safetensors"),
        &device,
    )?;
    ...
    assert!(
        e_sdpa < 5e-3,
        "Rust manual path must reproduce Python use_pytorch_sdpa=True: {e_sdpa}"
    );
...
fn generate_interleaved_parity() -> anyhow::Result<()> {
    ...
    let manifest = PathBuf::from(env!("CARGO_MANIFEST_DIR"));
    let device = Device::Cpu;
    let r = candle_core::safetensors::load(
    ...
    assert_eq!(
        seq_mod, want_mod,
        "generation modality sequence diverged from Python"
    );
    assert_eq!(
        text_vals,
        g.get("text_vals")
            ...
            .to_dtype(DType::I64)?
            .to_vec1::<i64>()?,
        "text token ids diverged from Python"
    );
    let want_audio = g.get("audio_vals").unwrap().to_dtype(DType::I64)?;
    let got_audio = Tensor::from_iter(audio_vals.iter().flatten().copied(), &device)?
        .reshape(want_audio.dims())?;
    assert_eq!(
        got_audio.to_vec2::<i64>()?,
        want_audio.to_vec2::<i64>()?,
        "audio frame codes diverged from Python"
    );
    println!(
        "generate parity: {} tokens ({} text, {} audio) — all ids match Python exactly",
        seq_mod.len(),
        text_vals.len(),
        audio_vals.len()
    );
...
fn conformer_streaming_parity() -> anyhow::Result<()> {
    ...
    let g = candle_core::safetensors::load(
        manifest.join("parity/golden/conformer_streaming_refs.safetensors"),
        &device,
    )?;
    ...
    let (out, out_len, next_ch, next_time, next_len) =
        enc.forward_streaming(&mel, None, &cch, &ctime, &clen)?;

    let want_out = g.get("out").expect("out");
    assert_eq!(
        out.dims(),
        want_out.dims(),
        "streaming output shape vs Python"
    );
    let e = rel_err(&out, want_out);
    println!(
        "streaming output vs Python: {e:.3e} (shape {:?})",
        out.dims()
    );
    ...
    assert_eq!(
        next_time.dims(),
        g.get("next_time").expect("next_time").dims(),
        "next_time shape vs Python"
    );
...
fn subsampling_schemes_parity() -> anyhow::Result<()> {
    ...
    let device = Device::Cpu;
    let g = candle_core::safetensors::load(
        manifest.join("parity/golden/subsampling_schemes_refs.safetensors"),
        &device,
    )?;
...
fn create_masks_parity() -> anyhow::Result<()> {
    ...
    let device = Device::Cpu;
    let g = candle_core::safetensors::load(
        manifest.join("parity/golden/create_masks_refs.safetensors"),
        &device,
    )?;
    ...
    let diff = |a: &Tensor, b: &Tensor| -> anyhow::Result<i64> {
        Ok((a.to_dtype(DType::I64)? - b.to_dtype(DType::I64)?)?
            .abs()?
            .sum_all()?
            .to_scalar::<i64>()?)
    };
    for (name, sam, style, acs, use_off) in cases {
        ...
        let (pad_mask, att_mask) =
            ConformerEncoder::build_masks(sam, style, acs, &plen, m, offset, &device)?;
        let want_pm = g.get(&format!("{name}.pad_mask")).expect("pad_mask");
        assert_eq!(
            diff(&pad_mask, want_pm)?,
            0,
            "{name} pad_mask mismatch vs Python"
        );
...
fn abs_attention_parity() -> anyhow::Result<()> {
    ...
    let device = Device::Cpu;
    let g = candle_core::safetensors::load(
        manifest.join("parity/golden/mha_abs_refs.safetensors"),
        &device,
    )?;
    let q = g.get("q").expect("q").clone();
...
fn conformer_stages_parity() -> anyhow::Result<()> {
    ...
    let manifest = PathBuf::from(env!("CARGO_MANIFEST_DIR"));
    let device = Device::Cpu;
    let stages = candle_core::safetensors::load(
        manifest.join("parity/golden/conformer_stages.safetensors"),
        &device,
    )?;
    let refs =
        candle_core::safetensors::load(manifest.join("parity/golden/refs.safetensors"), &device)?;
    ...
    for (name, got) in [
        ...
        let want = stages
            .get(name)
            .unwrap_or_else(|| panic!("{name} missing in ref"));
        let e = rel_err(got, want);
        println!(
            "{name:8} rust {:?}  ref {:?}  rel-err {e:.3e}",
            got.dims(),
            want.dims()
        );
...
fn mimi_decode_smoke() -> anyhow::Result<()> {
    ...
    println!(
        "mimi decode: codes {:?} -> waveform {:?}  ({} samples, max|amp| {:.4})",
        codes.dims(),
        wav.dims(),
        n,
        max
...
fn mimi_streaming_decode_smoke() -> anyhow::Result<()> {
    ...
    assert_eq!(
        n_stream,
        (t - warmup_none) * 1920,
        "unexpected streaming sample count"
    );
    Ok(())
}
...
fn mimi_encode_smoke() -> anyhow::Result<()> {
    ...
    println!(
        "mimi encode: {sr} Hz wav (1,1,{n}) -> codes {:?}",
        codes.dims()
    );
...
fn conformer_streaming_inventory() -> anyhow::Result<()> {
    ...
    println!(
        "cache shapes: last_channel {:?} last_time {:?} len {:?}",
        cc.dims(),
        ct.dims(),
        cl.dims()
    );
    ...
    assert_eq!(
        cc.dim(2)?,
        enc.streaming_cfg().last_channel_cache_size as usize
    );
    ...
    let (enc_out, elen_out, cache_out) =
        enc.streaming_post_process(encoded, elen, Some(cc.clone()), false)?;
    assert_eq!(enc_out.dim(2)?, valid, "encoded trimmed to valid_out_len");
    assert_eq!(
        elen_out.to_vec1::<i64>()?,
        vec![valid as i64],
        "length clamped"
    );
...
fn training_gradients_reach_attention_and_norms() -> anyhow::Result<()> {
    ...
    assert!(
        miss.is_empty(),
        "conformer params with NO gradient (autograd severed): {:?}",
        &miss[..miss.len().min(8)]
    );
    ...
    let (h2, t2, mut miss2) = severed(
        &tl.model.backbone_forward_embeds(&embeds)?.sum_all()?,
        "lfm.",
    )?;
    ...
    assert!(
        miss2.is_empty(),
        "backbone params with NO gradient: {:?}",
        &miss2[..miss2.len().min(8)]
    );
    ...
    assert!(
        sub_total > 0,
        "expected to find subsampling conv weights by name"
    );
    assert_eq!(
        sub_have, sub_total,
        "subsampling conv weights with NO gradient after full backward"
    );
    Ok(())
}
...
fn batched_logits_python_parity() -> anyhow::Result<()> {
    ...
    assert_eq!(
        tl.dims(),
        want_tl.dims(),
        "text_logits shape (row-0 bug → half the supervised positions)"
    );
    ...
    println!(
        "batched vs Python: text_logits {:?} rel-err {e_tl:.3e}; loss rel-err {e_loss:.3e}",
        tl.dims()
    );
...
fn batched_prefill_logits_self_consistency() -> anyhow::Result<()> {
    ...
    println!(
        "batched prefill rel-err row0 {e0:.3e} row1 {e1:.3e}  dims {:?}",
        p2.dims()
    );
    assert!(
        e0 < 1e-5 && e1 < 1e-5,
        "batched prefill not row-consistent: {e0} {e1}"
    );
    ...
    assert_eq!(
        ttok2.to_vec1::<i64>()?,
        dup(&ttok1)?.to_vec1::<i64>()?,
        "batched text labels"
    );
    assert_eq!(
        atok2.to_vec1::<u32>()?,
        dup(&atok1)?.to_vec1::<u32>()?,
        "batched audio labels"
    );
    Ok(())
}
...
fn depthformer_parity() -> anyhow::Result<()> {
    ...
    let manifest = PathBuf::from(env!("CARGO_MANIFEST_DIR"));
    let device = Device::Cpu;
    let refs = candle_core::safetensors::load(
        manifest.join("parity/golden/depthformer_refs.safetensors"),
        &device,
    )?;
...
fn backbone_parity() -> anyhow::Result<()> {
    ...
    let device = Device::Cpu;
    let refs = candle_core::safetensors::load(
        manifest.join("parity/golden/backbone_refs.safetensors"),
        &device,
    )?;
...
fn front_end_parity() -> anyhow::Result<()> {
    ...
    let refs_path =
        PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("parity/golden/refs.safetensors");
    let device = Device::Cpu;
