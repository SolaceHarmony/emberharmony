...
- **As-built** sections describe what is in the working tree *now*, verified against the
  source (`src/flashkern/`, `src/model/lfm2_hf.rs`, `src/model/lfm2_audio.rs`,
  `src/bf16_gemm.rs`, `csrc/flashkern_*.cpp`). If it says "as-built", the code does it.
...
The kernel-level companion is `csrc/FLASHKERN.md` (the Metal-idiom → NEON/AVX opcode map and
...
- **As-built.** Weights are memory-mapped by candle's safetensors `VarBuilder` at load
  ...
  `(ptr, len)` into candle's `Arc`-heap CPU storage — `src/flashkern/decode.rs`). No
...
- **`SpinBarrier`** (`src/flashkern/decode.rs`): a spinning generation barrier with
...
Verified in source. See `csrc/FLASHKERN.md` for the four flashkern kernels on the live path.
