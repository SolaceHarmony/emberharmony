...
(`csrc/flashkern_neon.cpp` / `csrc/flashkern_x86.cpp`, bridged by `src/flashkern/{neon,x86}.rs`)
or, for the multi-stage kernels, as a pure-Rust port under a faithful CPU model of the GPU
dispatch (`src/flashkern/fanout.rs`: threadgroup grid → rayon, simdgroup lanes → a scoped
thread team, `threadgroup_barrier` → `std::sync::Barrier`). The double-double toolkit
(`double_double.metal`) lives at `src/flashkern/dd.rs`. The build is wired in `build.rs`.
...
* the **fused FFN residual block** (`src/flashkern/decode.rs`): on CPU decode,
...
* the **pure-NEON depthformer frame decoder** (`DepthDecode`, `src/flashkern/decode.rs`):
...
| `double_double.metal` toolkit | `src/flashkern/dd.rs` (formulation-faithful); SIMD `lfm_dd_sum/dot_f32` | dd.rs; both `.cpp` |
...
(`DepthDecode`, `src/flashkern/decode.rs`) executes between spin barriers — the CPU analog of
...
* **Rust** (`src/flashkern/neon.rs`): the `#[cfg(test)] mod tests` is itself aarch64+flashkern-gated, so it
...
## x86-64 sibling (`csrc/flashkern_x86.cpp` + `src/flashkern/x86.rs`)
