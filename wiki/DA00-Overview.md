...
flowchart TD
    ...
    click types "DA04-Data-Types"
    click mapper "DA02-Chat-Mapper"
    click preprocess "DA03-Preprocess-Arrow"
    click dataloader "DA01-DataLoader"
```
...
- **Downstream** — the [`../model/lfm2_audio.md`](MD01-LFM2AudioModel) backbone (`_prefill` scatters `text`/`audio_in`/`audio_out` embeddings by `modality_flag`; `forward` builds the CE-loss masks from `supervision_mask`) and the [`../trainer.md`](CO04-Trainer) loop (moves the bundle with `.to(device)` and calls the model). The folder hands off exactly the `LFM2AudioModelInput` contract; everything past the hand-off is the **training/loss** subsystem, which is itself off the LFM2-Audio inference path.
