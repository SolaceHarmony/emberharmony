...
# QZ01 · SplitResidualVectorQuantizer
...
## Wiring
**Upstream (feeds this):** `MimiModel`'s encoder + framerate downsample produces the 12.5 Hz latent `(B, 512, T)` model dtype that enters `Split…encode`; on decode, `MimiModel` passes `(B, 8, T)` int codes into `Split…decode`. The quantizer is owned and driven by [moshi_compression](MM01-Mimi-Codec). Its build/hyperparameters (`_quantizer_kwargs`: `dim=256, n_q=32, bins=2048`, `set_num_codebooks(8)`) come from [moshi_loaders](MM02-Mimi-Loaders).

**Downstream (consumes this output):**
- Encode codes `(B, 8, T)` int → bubble up through [moshi_compression](MM01-Mimi-Codec) `MimiModel.encode` to the training data path [data_mapper](DA02-Chat-Mapper), which appends `EOAudio=2048` and stores the `audio_out` target `(8, L+1)` int64.
- Decode latent `(B, 512, T)` model dtype → consumed inside [moshi_compression](MM01-Mimi-Codec) by the framerate upsample + decoder transformer + SEANet decoder to produce waveform; ultimately the [core_processor](CO01-Processor-ChatState) `decode()` dispatch and the demo streaming-decode path.
- Interface/dataclass contract: `BaseQuantizer` / `QuantizedResult` from [moshi_quant_base](QZ03-Quantizer-Base); the codebook engine is [moshi_core_vq](QZ02-VQ-Core).
