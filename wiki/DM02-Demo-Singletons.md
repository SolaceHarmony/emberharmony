...
- HF snapshot `LiquidAI/LFM2.5-Audio-1.5B` (`model.safetensors` bf16, `tokenizer-…-checkpoint125.safetensors` fp32, `tokenizer.json`, `config.json`) → resolved by [core_utils](CO03-Utils) `get_model_dir` (snapshot_download).
- Constructs [core_processor](CO01-Processor-ChatState) (tokenizer + mel + Mimi dispatch + `ChatState`) as `proc`, bf16/cuda.
- Constructs [model_lfm2_audio](MD01-LFM2AudioModel) (top model: prefill + `generate_interleaved` + text head + depthformer) as `lfm2_audio`, bf16 weights `(…,2048)`, cuda.
- Aliases [moshi_compression](MM01-Mimi-Codec) `MimiModel` (held by the processor) as `mimi`.
- Warmup feeds int64 codes `(1,8,1)` into `mimi.decode` → f32 `(1,1,1920)` @24 kHz, discarded.

**Downstream (who imports these singletons):**
- [demo_chat](DM01-Realtime-Chat) — the **only** consumer: `from .model import lfm2_audio, mimi, proc` (`chat.py:11`). It drives `lfm2_audio.generate_interleaved(**chat, …)` (yielding text tokens `(1,)` int64 and audio frames `(8,)` int), `mimi.decode(t[None,:,None])` for `(8,)`→`(1,1,1920)` f32 streaming audio under `mimi.streaming(1)`, and `proc.text.decode(...)` to detokenize text. `proc` also seeds `ChatState(proc)`.
...
- **No dtype gotchas in this file** — it introduces no casts. The bf16-weight / f32-CPU-floor / f64-mel / RMSNorm-bf16-order subtleties all live in the loaded components ([model_lfm2_backbone](MD02-LFM2-Backbone), [conformer_processor](CF04-Mel-Frontend), [model_transformer](MD04-Depthformer)), not here.
