...
- [core_processor](CO01-Processor-ChatState) — `ChatState(proc)`: the `proc` provides the tokenizer + mel front-end + Mimi handle; `add_audio` calls `proc.audio` (mel `(128,T')` bf16) and `add_text` calls `proc.text.encode` (int64 ids). `**chat` splats the 5-key bundle into the model.

**Downstream**
- [model_lfm2_audio](MD01-LFM2AudioModel) — `generate_interleaved(**chat,…)` consumes `{text int64 (1,L), audio_in bf16 (128,ΣT'), audio_in_lens int64, audio_out int (8,·), modality_flag int64 (1,L)}`; yields text `(1,)` int64 and audio frames `(8,)` int.
- [core_processor](CO01-Processor-ChatState) — `chat.append(...)` writes the assistant's emitted `text (1,·) int64` / `audio_out (8,·) int` / `modality_flag` back into the same `ChatState`.
- [moshi_compression](MM01-Mimi-Codec) — `mimi.decode(codes (1,8,1) int)` under `mimi.streaming(1)` → `(1920,)` f32 @ 24 kHz waveform chunk (Rust: `mimi.decode_step`).
