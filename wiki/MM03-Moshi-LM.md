...
# MM03 · LMModel + LMGen (Moshi 7B LM)
...
- Mimi codes int `[B, n_q, T]` from the codec ← [MimiModel (compression.py)](MM01-Mimi-Codec) (encode side, training/`run_inference`).
- Conditioning tensors f32 ← `ConditionProvider`/`ConditionFuser` (`moshi_cond_base`, summed/cross-attn), gated by `fuser`.
- Sub-components it imports: `ScaledEmbedding` + `_delay_sequence`/`_undelay_sequence`/`_init_layer` ← [lm_utils.py](MM04-Moshi-LM-Utils); `sample_token` ← `moshi/utils/sampling.py`; `StreamingTransformer`/`create_norm_fn` ← `moshi/modules/transformer.py`; `CUDAGraphed` ← `moshi/utils/compile.py`.

**Downstream (consumes this output — off LFM2-Audio path):**
- `LMGen.step` audio frame int `[B,8,1]` → [MimiModel (compression.py)](MM01-Mimi-Codec)`.decode` → waveform f32 @24kHz, inside `moshi/server.py` / `moshi/run_inference.py`.
- `LMOutput` (train) → the Moshi trainer's cross-entropy (not LFM2-Audio's `core_trainer`).
- `TTSModel` ([tts.py](MM05-Moshi-TTS)) wraps `LMModel`+`LMGen` for script-driven TTS (off-path).

LFM2-Audio's actual audio LM head is **not** this file — it is [lfm2_audio.py](MD01-LFM2AudioModel)'s depthformer over [lfm2_backbone.md](MD02-LFM2-Backbone); this `LMModel` has no consumer on the LFM2-Audio tensor path.
