...
# MO04 · ConvDownsample/Upsample 25<->12.5Hz
...
## Wiring
**Upstream (encode side):** `[moshi_transformer](MO03-Codec-Transformer)` (the Mimi `encoder_transformer`, `ProjectedTransformer`) emits the 25 Hz latent `(B,512,T@25Hz)` model-dtype → **ConvDownsample1d**. Ultimately fed by `[moshi_seanet](MO01-SEANet)` `SEANetEncoder` (waveform→`(B,512,T@25Hz)`). Orchestrated by `[moshi_compression](MM01-Mimi-Codec)` `MimiModel.encode/_to_framerate`.

**Downstream (encode side):** ConvDownsample1d out `(B,512,T/2@12.5Hz)` model-dtype → `[moshi_vq](QZ01-Split-RVQ)` `SplitResidualVectorQuantizer.encode` (which input-projects 512→256, runs RVQ, emits int codes).

**Upstream (decode side):** `[moshi_vq](QZ01-Split-RVQ)` `.decode` emits latent `(B,512,T'@12.5Hz)` model-dtype → **ConvTrUpsample1d**.

**Downstream (decode side):** ConvTrUpsample1d out `(B,512,2·T'@25Hz)` model-dtype → `[moshi_transformer](MO03-Codec-Transformer)` (Mimi `decoder_transformer`) → `[moshi_seanet](MO01-SEANet)` `SEANetDecoder` → waveform `(B,1,T)`@24kHz. Top-level consumer is `[moshi_compression](MM01-Mimi-Codec)` `MimiModel.decode/decode_step`, which is what `[core_processor](CO01-Processor-ChatState)` and `[demo_chat](DM01-Realtime-Chat)` call to turn generated 8-code frames into 24 kHz audio.
