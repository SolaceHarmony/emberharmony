...
- Upstream: raw waveform `(B,1,T)` f32 @ 24 kHz from `mimi.encode`, fed by [moshi_compression](MM01-Mimi-Codec) (`MimiModel.encode`, which `pad_for_conv1d`s to a multiple of frame_size first).
- Downstream: latent `(B,512,T/960)` model dtype @ 25 Hz → Mimi's encoder transformer → `ConvDownsample1d` ([moshi_resample](MO04-Framerate-Resample), 25→12.5 Hz) → [moshi_vq](QZ01-Split-RVQ) `SplitResidualVectorQuantizer` (512→256 proj, cdist-argmin RVQ) → codes `(B,8,·)`.

**Decoder (inference path):**
- Upstream: latent `(B,512,t)` model dtype @ 25 Hz from `quantizer.decode` → `ConvTrUpsample1d` ([moshi_resample](MO04-Framerate-Resample), 12.5→25 Hz) → decoder transformer, all orchestrated by [moshi_compression](MM01-Mimi-Codec) (`MimiModel.decode` / `_decode_frame`).
- Downstream: waveform `(B,1,t·960)` f32 @ 24 kHz → back to [moshi_compression](MM01-Mimi-Codec) which returns it to [core_processor](CO01-Processor-ChatState) `decode()` and ultimately the demo audio sink ([demo_chat](DM01-Realtime-Chat), which plays each `(1,1,1920)` chunk inside `mimi.streaming(1)`).

The 8-code frames that feed the decode side originate at [model_lfm2_audio](MD01-LFM2AudioModel)'s depthformer audio head (`frame (8,)` int, codes 0..2047, 2048=EOAudio) — but those go through the quantizer/resampler first, not directly into SEANet.
...
| `StreamingConv1d`/`StreamingConvTranspose1d` | `StreamableConv1d`/`StreamableConvTranspose1d` ([moshi_conv](MO02-Streaming-Conv)) |
...
- **`hop_length=960`, not 1920.** SEANet alone is 24000/960 = 25 Hz. The codec's 1920-sample / 12.5 Hz frame is SEANet's 960 **times** the `ConvDownsample1d` stride-2 ([moshi_resample](MO04-Framerate-Resample)). Don't attribute the 1920 to SEANet.
