...
# QZ02 · EuclideanCodebook + VQ core
...
## Wiring
**Upstream (encode path, training-data only):** SEANet-encoded + downsampled latent `[B,512,T]@12.5Hz` from [`MimiModel`](MM01-Mimi-Codec) → `vq.py` `input_proj` `Conv1d 512→256` → residual `[B,256,T]` f32 enters `ResidualVectorQuantization.encode` here. Driven by [`moshi_vq`](QZ01-Split-RVQ) (`SplitResidualVectorQuantizer.encode`).

**Upstream (decode path, live inference):** generated 8-code audio frame `(8,)` int from the depthformer audio head ([`model_lfm2_audio`](MD01-LFM2AudioModel)) → demo reshapes to `[1,8,1]` → [`MimiModel.decode`](MM01-Mimi-Codec) → `vq.py` `SplitResidualVectorQuantizer.decode` splits codes `[B,8,T]` into semantic/acoustic → `ResidualVectorQuantization.decode` here, **codes int (u32 in Rust)**.

**Downstream (decode path):** summed latent `[B,256,T]` model dtype → `vq.py` `output_proj` `Conv1d 256→512` → upsample → decoder transformer → SEANet decoder → waveform `f32 @24kHz`, all inside [`MimiModel`](MM01-Mimi-Codec). The immediate consumer of `core_vq`'s decode output is [`moshi_vq`](QZ01-Split-RVQ)'s `output_proj`.
