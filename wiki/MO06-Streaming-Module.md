...
## Wiring
**Upstream (who enters streaming on these):** the LFM2-Audio decode loop opens `mimi.streaming(1)` / `streaming_forever(1)` around the Mimi codec — driven by [core_processor](MD01-LFM2AudioModel)'s `decode()` dispatch and the demo loop ([demo_chat](DM01-Realtime-Chat), `moshi/server.py`). Edge carried: the per-frame audio codes `int/u32 (B,8,T)` that the Mimi `forward`/`decode` consumes inside the `with` block.

**This base class is mixed into (subclassed by):**
- [moshi_compression](MM01-Mimi-Codec) — `CompressionModel`/`MimiModel` (`compression.py:40,105`); `_MimiState` carries the CUDA-graphed enc/dec transformers.
- [moshi_seanet](MO01-SEANet) — `SEANetEncoder`/`Decoder`/`ResnetBlock` are `StreamingContainer`s (`seanet.py:20,96,242`).
- [moshi_conv](MO02-Streaming-Conv) — `StreamingConv1d`/`ConvTranspose1d` (`conv.py:172`) hold the causal ring-buffer + partial-frame state.
- [moshi_transformer](MO03-Codec-Transformer) — `StreamingTransformer`/`Layer`/`MultiheadAttention` (`transformer.py:328,586,789`) hold the KV cache + RoPE offset.
- [moshi_lm](MM03-Moshi-LM) — `LMModel`/`LMGen` (`lm.py:49,550`) are off the LFM2-Audio compute path (reference Moshi LM) but reuse the identical state API.

**Downstream (consumes this component's output / API):** the modules above; their decoded waveform `f32 @24kHz (B,1,L)` exits the streaming context back to [core_processor](MD01-LFM2AudioModel) → audio sink.
