...
**Upstream:**
- [data_dataloader](DA01-DataLoader) — `DataLoader` + `lfm2_collator` produce the collated `LFM2AudioModelInput` batches (int64 text `(B,L)`, bf16 mel audio_in, int audio_out codes `(B,L,C)`, bool supervision_mask) fed to `train_step`/`validate`.
- [data_types](DA04-Data-Types) — the `LFM2AudioModelInput`/`LFM2AudioModelOutput` dataclasses (and `.to(device)`) that cross every trainer boundary.

**Driven (the model the trainer optimizes — the loss source, not "downstream output"):**
- [model_lfm2_audio](MD01-LFM2AudioModel) — `train_step`/`validate` call its `forward(batch)`; it returns `out.loss` (f32 scalar) and holds the `audio_loss_weights` buffer + per-modality multipliers. The trainer's gradients flow back into this model's bf16 params.

**Downstream (consumers of the trainer's *output* — produced weights):**
- [model_lfm2_audio](MD01-LFM2AudioModel) — the checkpointed/`save_model`'d bf16 safetensors are what `LFM2AudioModel.from_pretrained` later loads for inference.
...
- **The loss lives in the model, not the trainer.** Any per-codebook weighting, modality multiplier, or `ignore_index=-100` masking question must be answered at [model_lfm2_audio](MD01-LFM2AudioModel); the trainer only sums/means the returned f32 scalar. The Rust upcasts the val/log loss to f32 explicitly (`to_dtype(F32)`, `:468`/`:489`) before reducing.
