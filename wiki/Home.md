# LFM2.5-Audio — Architecture Wiki

A code-cross-referenced architecture of Liquid AI's **LFM2.5-Audio** (the vendored Python
reference) and its faithful **Rust port** (`liquid-audio-rs`). Each component page carries a
stable **code** (e.g. `MD04`), maps to its Python `file:line`, and links its neighbours — the
[Architecture map](Architecture-Map) renders the full wiring as a clickable mermaid graph.

**Start here:** [Architecture map](Architecture-Map) · [Cross-reference matrix](Cross-Reference) ·
[Runtime questions](Runtime-Questions) · [Codec deep-dive](Codec-Deep-Dive)

This wiki is generated from `wiki/**` and published via `.github/workflows/publish-wiki.yml`.
