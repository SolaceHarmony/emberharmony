<!-- topic: Core -->
# CO03 · utils — LFMModality, mel2emb_len, get_model_dir
...
- The conformer mel front-end produces a log-mel of width `T` (f32/f64 computed,
  ...
  [conformer_processor](CF04-Mel-Frontend) (mel `(128,T)`) and the
  8×-subsample length contract from [conformer_subsampling](CF05-Subsampling).
- The on-disk checkpoint (bf16 weights) is what `get_model_dir` resolves a
  directory for; consumed by the loaders in
  [core_processor](CO01-Processor-ChatState) and [model_lfm2_audio](MD01-LFM2AudioModel).

**Downstream (who consumes these outputs):**
- [core_processor](CO01-Processor-ChatState) — imports `LFMModality`, `get_model_dir`,
  `mel2emb_len`; builds/extends the int64 `modality_flag (1,L)` and resolves the
  checkpoint dir.
- [model_lfm2_audio](MD01-LFM2AudioModel) — imports all four; `module_exists`
  ...
  `(1,L,2048)`.
- [data_mapper](DA02-Chat-Mapper) and [data_dataloader](DA01-DataLoader) —
  use `LFMModality` ints and `mel2emb_len` to build training modality/supervision
  sequences (int64) and pad with `LFMModality.TEXT`.
- [demo_chat](DM01-Realtime-Chat) — uses `LFMModality.TEXT`/`AUDIO_OUT` to tag the
