...
# MO03 · ProjectedTransformer (codec)
...
- Upstream: [moshi_seanet](MO01-SEANet) `SEANetEncoder` → encoder latent `[B,512,T']` model-dtype, `conv_layout` (channels-first), CUDA-graphed by [moshi_compression](MM01-Mimi-Codec).
- Downstream: [moshi_vq](QZ01-Split-RVQ) `SplitResidualVectorQuantizer` consumes the refined `[B,512,T']` (it owns the 512↔256 input/output projections).

**Decoder side (audio-out, the active LFM2-Audio detok-via-Mimi path):**
- Upstream: [moshi_vq](QZ01-Split-RVQ) `.decode()` → reconstructed latent `[B,512,T']` model-dtype, fed via [moshi_compression](MM01-Mimi-Codec) `decoder_transformer`.
- Downstream: [moshi_seanet](MO01-SEANet) `SEANetDecoder` → waveform `f32 @ 24kHz`.

The whole codec is driven by [core_processor](CO01-Processor-ChatState) (`decode()` / Mimi dispatch) and, in the streaming demo, by [demo_chat](DM01-Realtime-Chat) via `mimi.streaming(1)`. RoPE tables come from [moshi_rope](MO05-RoPE); streaming-state machinery from [moshi_streaming](MO06-Streaming-Module); CUDA-graph capture from [moshi_util_compile](MU02-CUDA-Graphs). Note: [moshi_lm](MM03-Moshi-LM) also imports `StreamingTransformer` from here but is **off-path** (different model).
