...
| `wrap_activation_checkpoint` (torch checkpoint) | `wrap_activation_checkpoint<T>(module: T) -> T` identity | **deliberate stub** | no autograd on inference path; preserved for 1:1 inventory (§2.5). |
...
| `SequenceModel(nn.Module, ABC)` | `trait SequenceModel` | **ABC → trait** | the standard Rust substitution. `fn new() where Self: Sized` keeps it object-safe. |
