...
- **Moshi server** [moshi_server](server.md) → ws binary frames: `b"\x01"+opus` (model speech) and `b"\x02"+utf8` (text tokens) → this client's `_recv_loop`. The server's own audio comes out of the Mimi codec [moshi_compression](models/compression.md) at 24 kHz, re-encoded to Opus by `sphn`.

**Downstream:**
- This client → ws binary `b"\x01"+opus` (mic) → **Moshi server** [moshi_server](server.md), which Opus-decodes to 24 kHz PCM and feeds the Mimi encoder + Moshi LM [moshi_lm](models/lm.md).
...
## Precision / gotchas
- **Not the LFM2-Audio path.** This talks to the Moshi 7B LM [moshi_lm](models/lm.md), a *different* model. Reading it tells you the Moshi server wire protocol, not anything about LFM2-Audio inference. The on-path realtime client is `demo/chat.py` ([demo_chat](../demo/chat.md)).
