...
No LFM2-Audio component ([core_processor](../../processor.md), [model_lfm2_audio](../../model/lfm2_audio.md), [model_transformer](../../model/transformer.md), [moshi_compression](../models/compression.md)) imports or instantiates this; the codec and backbone run full-precision.
...
- **Naming collision.** This "quantize" is **weight int8**, unrelated to the RVQ/codebook "quantize" in [moshi_vq](../quantization/vq.md)/[moshi_core_vq](../quantization/core_vq.md) and the `quantize=` flag on the codec's `set_num_codebooks` — do not conflate them. No EOAudio/special-token or off-by-one concerns apply here (no sequence logic in this file).
