...
- **Moshi websocket server** ([server.md](server.md)) → inbound messages on `/api/chat`: tagged **Opus audio (`0x01`)** decoded to f32 PCM `(M,)` @ 24 kHz, and **text (`0x02`)** as UTF-8 bytes. The server's `send_loop` produces exactly these tagged frames; this client is the symmetric peer of `moshi/client.py` ([client.md](client.md)) but with a WebRTC/gradio front-end instead of `sounddevice`.

**Downstream (what consumes this output):**
- **Moshi websocket server** ([server.md](server.md)) ← outbound tagged Opus audio (`b"\x01"+opus`) from `receive()`; the server's `recv_loop` decodes it and feeds Mimi-encoded codes into `lm_gen.step`.
...
This component does **not** touch the LFM2-Audio core ([../model/lfm2_audio.md], [../processor.md], [../detokenizer.md]) at all — it is a Moshi-server transport. The Mimi codec ([models/compression.md](models/compression.md)) is relevant only on the *server* side of the wire.
...
- **Wire protocol = 1-byte kind tag.** `0x01`=audio, `0x02`=text, prepended on send (`:56`) and consumed as `message[0]` on receive (`:65`). This must stay in lockstep with the server's framing ([server.md](server.md)); a tag mismatch routes audio into the text path or vice-versa.
