...
## Wiring
**Upstream (feeds q/k into rope):** the Mimi codec's encoder/decoder `StreamingTransformer` self-attention — `q,k,v` come from the fused in-projection inside [moshi_transformer](transformer.md) (`StreamingMultiheadAttention`, q/k of shape `[B,8,T,64]`, model dtype). `offset` (int64 `[B]`) comes from the streaming `_MHAState`. Those transformers are embedded in [moshi_compression](../models/compression.md) (MimiModel enc/dec), which itself runs inside the SEANet pipeline of [moshi_seanet](seanet.md).

**Downstream (consumes rotated q/k):** the rotated `(qo, ko)` flow straight back into `StreamingMultiheadAttention` of [moshi_transformer](transformer.md) — KV-cache completion then `F.scaled_dot_product_attention`. The codec result ultimately surfaces to [moshi_compression](../models/compression.md) `encode`/`decode`, i.e. the RVQ tokens consumed by [moshi_vq](../quantization/vq.md) and the waveform produced for [core_processor](../../processor.md). RoPE has no direct tensor consumer outside the attention that calls it.
