...
- mic/file wav f32 `(1, L)` → `add_audio` → routed to the mel front-end [conformer_processor](model/conformer/processor.md) as f32 `(1, L')` @16 kHz.
- generated text token (int64) + audio frame `(8,)` int + modality flag from [model_lfm2_audio](model/lfm2_audio.md)'s `generate_interleaved` → `append`.
- `LFMModality` enum, `mel2emb_len`, `get_model_dir` from [core_utils](model/utils.md).

**Downstream (consumes processor / ChatState output):**
- The five-tensor ChatState bundle (`text` int64 `(1,L)`, `audio_in` bf16 `(128,ΣF)`, `audio_in_lens` int64 `(k,)`, `audio_out` int64 `(8,m)`, `modality_flag` int64 `(1,L)`) → [model_lfm2_audio](model/lfm2_audio.md) `_prefill`/`generate_interleaved`.
- `decode((1,8,T))` int codes → [core_detokenizer](detokenizer.md) (LFM2.5) for ISTFT vocoding, or → [moshi_compression](moshi/models/compression.md) `MimiModel.decode` (v1/demo streaming) → f32 `(1,T')` @24 kHz.
- `mimi.encode` (data prep) routes to [moshi_compression](moshi/models/compression.md) for building `audio_out` targets.
