...
## Wiring
**Upstream:** [core_processor](processor.md) `decode()` feeds `audio_codes` `(1,8,T)` int64 — the EOAudio frame (code 2048) **must be stripped first** (the `[0,2047]` guard rejects it). Those codes are produced by the depthformer audio head in [model_lfm2_audio](model/lfm2_audio.md) (`_sample_audio_frame` → frame `(8,)` int, `2048`=EOAudio) and accumulated across the turn. The backbone inside this module is the HF [model_lfm2_backbone](model/lfm2_audio.md) (`Lfm2Model`, the Rust spec is `lfm2_hf.rs`).

**Downstream:** the f32 `(1,L)` 24 kHz waveform returns through [core_processor](processor.md) `decode()` to the caller (demo playback / file write). On the Rust side it is surfaced via the `AudioDetokenizer` trait so [core_processor](processor.md) dispatches LFM2-detok vs Mimi uniformly. The alternative audio-out backend for the same codes is [moshi_compression](../moshi/models/compression.md) (`MimiModel.decode`, v1/streaming).
