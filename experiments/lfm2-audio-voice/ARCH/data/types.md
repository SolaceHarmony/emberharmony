...
## Wiring
**Upstream (producers):**
- [`LFM2AudioChatMapper`](mapper.md) consumes `list[ChatMessage]` and emits
...
- [`preprocess_dataset`](preprocess.md) consumes `Iterable[list[ChatMessage]]`,
  ...
  bool sequences.
- [`LFM2DataLoader`](dataloader.md) reads the Arrow row back and emits `LFM2AudioRow`
  (padded to 4096) — edge: same six fields, `text`/`modality_flag`/`supervision_mask`
  now `(1, 4096)`.
- [`lfm2_collator`](dataloader.md) consumes `list[LFM2AudioRow]` and emits
  `LFM2AudioModelInput` — edge: six fields concatenated to `(B, …)`.

**Downstream (consumers):**
- [`LFM2AudioModel`](../model/lfm2_audio.md) — `.logits(batch)` / `.forward(batch)`
...
- [`Trainer`](../trainer.md) — `train_step(batch: LFM2AudioModelInput)` /
