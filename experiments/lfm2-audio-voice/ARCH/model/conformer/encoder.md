...
- `core_processor` ([processor.py](../../core/processor.md)) → mel front-end `conformer_processor` ([conformer/processor.py](processor.md)) produces per-clip log-mel `(128, F)` (stored bf16 in `ChatState`); `model_lfm2_audio` ([model/lfm2_audio.py](../lfm2_audio.md), `lfm2_audio.py:339-346`) pads the clip list and calls `self.conformer(padded_audio_in.mT.to(text_emb.dtype), audio_in_lens)`. **Edge:** mel features `(B, 128, T)` model dtype + `length` int64.

**Downstream — consumes this output:**
- `model_lfm2_audio` ([model/lfm2_audio.md](../lfm2_audio.md)) unpads via the length mask and concatenates to `audio_enc_concatenated` (`lfm2_audio.py:349-350`). **Edge:** `(ΣT', 512)` model dtype.
- `model_mlp` audio_adapter ([model/mlp.py](../mlp.md), `lfm2_audio.py:87,353`) — MLP **512→2048** (GELU exact-erf, hidden=2048) maps the encoder output to backbone width, giving `audio_in_emb (ΣT', 2048)` scattered into the AUDIO_IN slots of the backbone sequence. **Edge:** `(ΣT', 512)` model dtype → `(ΣT', 2048)`.
