...
## Wiring
**Upstream** — fed by:
- [conformer_subsampling](subsampling.md): the `dw_striding` ConvSubsampling 8× downsamples the `(B,128,T_mel)` mel into `(B, T', 512)` model-dtype features.
- [conformer_mha](mha.md)'s `RelPositionalEncoding`, which (in [conformer_encoder](encoder.md)) adds positional info to the subsampled features and emits the `pos_emb` table `(1, 2T'−1, 512)` consumed by every layer's `RelPositionMultiHeadAttention`.
- [conformer_encoder](encoder.md): owns the `nn.ModuleList` of 17 `ConformerLayer`s, threads `att_mask`/`pad_mask`/`pos_emb`, and (offline) calls each layer with `None` caches.

**Downstream** — consumes this output:
- [conformer_encoder](encoder.md): collects the stacked-layer output, transposes to `(B,512,T')`, and (no `out_proj`, `feat_out=-1`) returns `(B, T', 512)` model dtype.
- [model_mlp](../mlp.md): the `audio_adapter` MLP `Linear(512→2048) → GELU(erf) → Linear(2048→2048)` lifts the `_feat_out=512` encoding to backbone width `(ΣT', 2048)` model dtype (`lfm2_audio.py:87,346,353`).
- [model_lfm2_audio](../lfm2_audio.py.md)/[model_lfm2_backbone](../lfm2_audio.py.md): the adapted `audio_in_emb` is modality-scattered into the prefill sequence and fed to the LFM2 backbone.
