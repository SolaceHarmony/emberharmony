...
## Wiring
**Upstream** — [conformer_processor](processor.md) produces the mel features `(B,128,T)` bf16 (precision-sensitive f32/f64 front-end, stored bf16). They enter via [conformer_encoder](encoder.md), which transposes to `(B,T,128)` and calls `pre_encode(x, lengths)`. The caller path is [model_lfm2_audio](../lfm2_audio.md) `forward`: it splits per-clip, right-pads to `(N, T_max, 128)` (`lfm2_audio.py:341`), casts to model dtype, transposes back to `(N,128,T_max)` and hands it to `self.conformer(...)` (`lfm2_audio.py:346`).

**Downstream** — output `(B,T',512)` + lengths flows back into [conformer_encoder](encoder.md), which adds `RelPositionalEncoding` and runs the 17 `ConformerLayer`s ([conformer_modules](modules.md), [conformer_mha](mha.md)), returning `(B,512,T')`. The encoder result is masked/concatenated and fed to the `audio_adapter` [model_mlp](../mlp.md) (`MLP(512 → 2048)`, GELU-erf), whose output `audio_in_emb=(ΣT', 2048)` is scattered into the LFM2 backbone token stream ([model_lfm2_backbone](../lfm2_audio.md)). Net: `subsampling → encoder layers → audio_adapter → backbone`.
